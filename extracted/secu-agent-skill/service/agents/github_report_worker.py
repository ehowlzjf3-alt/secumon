"""Worker entrypoint for one GitHub report fanout target."""
...
from service.agents.github_report_agent import _handle_thread
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        raise SystemExit("usage: python -m service.agents.github_report_worker <evidence_dir>")
    ...
    try:
        ...
        write_worker_result(
            evidence_dir,
            status="error_crash",
            summary=f"github report worker failed: {e!r}",
            rc=1,
        )
        return 1
