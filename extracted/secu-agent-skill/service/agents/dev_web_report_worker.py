"""dev_web_report fanout worker entrypoint."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print("usage: dev_web_report_worker <evidence_dir>", file=sys.stderr)
        return 2
    ...
    if not thread:
        ...
        return 0
    try:
        result = asyncio.run(dev_web_report_agent._report_one_thread(thread, evidence_dir=evidence_dir))
        if result.get("error"):
        ...
        _write_worker_result(
            ...
            summary=f"thread_id={thread_id}: sent={result.get('saw_terminal')}",
