"""Worker entrypoint for one Confluence remediation recheck fanout target."""
...
from service.agents.confluence_recheck_agent import _handle_thread
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        raise SystemExit("usage: python -m service.agents.confluence_recheck_worker <evidence_dir>")
    ...
    try:
        ...
        write_worker_result(
            evidence_dir,
            status="error_crash",
            summary=f"confluence recheck worker failed: {e!r}",
            rc=1,
        )
        return 1
