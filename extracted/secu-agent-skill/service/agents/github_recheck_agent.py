"""GitHub E2E remediation recheck agent."""
...
log = logging.getLogger("service.agents.github_recheck")
    ...
    *,
    ...
    if _attempt_cap_exceeded(thread):
        state.github_report_thread_set_status(
            thread_id,
            "escalated",
            last_reason="github recheck attempt cap exceeded",
        )
        log.warning("[github-recheck] thread=%s attempt cap exceeded -> escalated", thread_id)
        return {"thread_id": thread_id, "final_status": "escalated", "escalated": True}

    ev_dir = evidence_dir or runtime.make_evidence_dir(f"github-recheck-{thread.get('repo')}")
    state.github_report_thread_set_status(
        thread_id,
        "rechecking",
        claimed_by=GITHUB_RECHECK_SESSION_ID,
    )
    ...
    if result.get("final_status") == "recheck_requested":
        retry_after = state.github_report_thread_schedule_recheck_retry(
            int(thread["id"]),
            reason="github recheck produced unknown results; retryable",
        )
...
def run_recheck_pass(
    *,
    ...
    component = COMPONENT_GITHUB_RECHECK
    state.heartbeat_upsert(component, phase=PHASE_GITHUB_RECHECK, pid=os.getpid())
    run_id = state.pipeline_run_start(component)
    handled = remediated = still_open = partially_remediated = escalated = retryable = sent = dry_run = errors = 0
    ...
    try:
        ...
        while True:
            ...
            if thread is None:
                break
            handled += 1
            state.heartbeat_upsert(component, phase=PHASE_GITHUB_RECHECK, detail=str(thread.get("repo")), pid=os.getpid())
            try:
                ...
                errors += 1
                log.warning("[github-recheck] thread=%s failed: %r", thread.get("id"), e)
        ...
        detail = (
            ...
            f"escalated={escalated} retryable={retryable} "
            f"sent={sent} dry_run={dry_run} errors={errors}"
        )
        return {
            "guidance": guidance,
            "handled": handled,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="GitHub E2E remediation recheck agent")
    ...
    log.info("[github-recheck] pass done %s", res)
    return 0
