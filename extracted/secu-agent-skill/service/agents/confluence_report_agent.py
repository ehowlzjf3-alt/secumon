"""Confluence E2E report agent — space findings to remediation report."""
...
log = logging.getLogger("service.agents.confluence_report")
...
def _ensure_evidence_dir(thread: dict[str, Any], evidence_dir: Path | None) -> Path:
    ...
    safe = "".join(ch if ch.isalnum() or ch in "-_." else "_" for ch in str(thread.get("space_key") or ""))[:48]
    out = root / f"{time.strftime('%Y%m%dT%H%M%S')}-{uuid.uuid4().hex[:6]}-confluence-report-{safe}"
...
def _write_report_artifact(report: dict[str, Any], *, evidence_dir: Path | None = None) -> str | None:
    if evidence_dir is None:
        return None
    out_dir = evidence_dir / "confluence_reports"
    out_dir.mkdir(parents=True, exist_ok=True)
    safe = "".join(ch if ch.isalnum() or ch in "-_." else "_" for ch in str(report.get("space_key") or ""))[:80]
...
def run_report_pass(
    *,
    ...
    state.heartbeat_upsert(component, phase=PHASE_CONFLUENCE_REPORT, pid=os.getpid())
        ...
        while True:
            ...
            try:
                ...
                if report.get("skipped_empty_report"):
                    skipped_empty += 1
                    log.warning(
                        "[confluence-report] space=%s skipped empty report",
                        thread.get("space_key"),
                    )
                    continue
                reports += 1
                ...
                if report.get("delivery_error"):
                    errors += 1
                log.info(
                    "[confluence-report] space=%s findings=%s",
                    thread.get("space_key"),
                    report.get("finding_count"),
                )
            except Exception as e:  # noqa: BLE001
                errors += 1
                log.warning("[confluence-report] thread=%s failed: %r", thread.get("id"), e)
                state.confluence_report_thread_set_status(
                    int(thread["id"]),
                    "reported",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Confluence E2E report agent")
    ...
    log.info("[confluence-report] pass done %s", res)
    return 0
