---
name: dev_web_lead
description: dev/stage 웹 타깃 큐 리드 — 큐를 보고 어디를 볼지 판단하고 검토원에게 위임한다
task_type: dev_web_lead
when_to_use: dev/stage 웹 타깃 큐 를 훑으며 무엇을 먼저 볼지 정해야 할 때(본문 열람은 검토원이 한다)
input_keys: []
---
...
위임 대상은 `dev_web_inspect` 하나다. 계약이 `agents_dir` 를 이 도메인 디렉터리로
