...
최신 Checkpoint374: C03 복구 관리 전파·실제 CLI5개 추가로 고유238개, C05 도구/문맥·MCP의 추가461개로 선택 고유662개를 확인했다. 실제 중단과 주입 오류의 범위, 각 실행의 소스 지문을 구분해 보존한다. [현재 결과와 남은 범위](../design/chapters/C05-ordered-verification-result.md). 아래 이전 단위 설명과 수치는 당시 이력이다. 전체 목표·운영 인수는 미완료다.
