...
최신 Checkpoint378: C08 동료·반론·자원 첫 로컬 인수에서 선택 고유199개와 관련 회귀47개를 확인했다. 일반 동료 세션과 도구 계약, 자원 준비 진전과 같은 task의 제한 재시도를 교정했다. [현재 결과와 남은 범위](../design/chapters/C08-ordered-verification-result.md) · [다음 작업](../design/NEXT-STEPS.md). 아래 이전 단위 수치와 당시 다음 작업은 이력이다. 각 빌드 기록을 최종 한 소스의 전체 재실행으로 합치지 않으며 전체 goal과 실제 모델·운영 인수는 미완료다.
