...
if (process.argv.length > 3) throw new Error('usage: node C05-source-read-comparison-probe.mjs [OUTPUT_JSON]');
assert.equal(process.versions.node.split('.')[0], '24');
const runtimeRoot = fileURLToPath(new URL('../', import.meta.url));
const scriptPath = fileURLToPath(import.meta.url);
const outputPath = resolve(process.argv[2] ?? join(runtimeRoot, 'evidence/C05-source-read-before2.json'));
...
const report = {
  ...
  conditions: [
    ...
    'No cache control, throughput/latency benchmark, tokenizer or model-quality evaluation. Compare logical port counts only between recorded source builds. Single elapsed observations include forwarding/stack overhead.',
...
try {
  ...
  // Each observation pins the current source and compiled build; before and after may differ.
  report.pinMethod = 'verifyEvaluationBuild before and after, no historical pin';
  ...
  stores = await openAgentStores(new FileAgentProfileStore(join(fixtureDirectory, 'engine')), join(fixtureDirectory, 'agent'), undefined, { identityRegistryDirectory: join(fixtureDirectory, 'registry') });
  ...
  assert(report.phases[0].events.some(event => event.key === 'session.history' && event.history.entries === 1));
