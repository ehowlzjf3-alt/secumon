...
function historicalCounts(stage) {
  const path = `runtime/evidence/C05-mcp-offline-${stage}`, record = JSON.parse(read(path + '.json')), values = logCounts(read(path + '.log'));
...
const firstNativePath = 'runtime/evidence/C05-mcp-offline-linux-nas-20260907/attempt-1/result.json';
const firstNativeLogPath = 'runtime/evidence/C05-mcp-offline-linux-nas-20260907/attempt-1/all-tests.log';
const priorMatches = proof.priorAttempts.filter(item => item.nativeAttempt === 1);
assert.equal(priorMatches.length, 1); const prior = priorMatches[0];
assert.equal(prior.evidence, firstNativePath); assert.equal(prior.status, 'failed');
for (const path of [firstNativePath, firstNativeLogPath]) assert.equal(proof.files.filter(item => item.path === path).length, 1);
const firstNative = JSON.parse(read(firstNativePath)), firstNativeLog = read(firstNativeLogPath);
assert.equal(firstNative.status, prior.status); assert.equal(firstNative.finishedAt, prior.finishedAt);
assert.ok(Number.isFinite(Date.parse(prior.finishedAt))); assert.deepEqual(firstNative.buildPin, prior.sourceAndBuild);
assert.equal(firstNative.environment.platform, 'linux'); assert.match(firstNative.environment.node, /^v24\./);
const failedSteps = firstNative.steps.filter(item => item.name === 'all-tests'); assert.equal(failedSteps.length, 1);
const failedAll = failedSteps[0], firstNativeCounts = logCounts(firstNativeLog);
assert.equal(failedAll.status, 'failed'); assert.equal(failedAll.exitCode, 1); assert.equal(failedAll.signal, null);
assert.equal(failedAll.timedOut, false); assert.equal(failedAll.logFlushCompleted, true);
assert.deepEqual(failedAll.tapSummary, firstNativeCounts); assert.ok(firstNativeCounts.fail > 0);
const failureNames = [...firstNativeLog.matchAll(/^not ok \d+ - (.+)$/gm)].map(match => match[1]);
assert.equal(failureNames.length, firstNativeCounts.fail);
const nativeAttemptHistory = { nativeAttempt: 1, result: firstNativePath, log: firstNativeLogPath,
  sourceAndBuild: prior.sourceAndBuild, finishedAt: prior.finishedAt, node: firstNative.environment.node, counts: firstNativeCounts, failureNames };

// These local supplements are verified here, separately from the final proof's files.
const separateLocalEvidence = new Map();
function supplemental(path) {
  assert.equal(proof.files.some(item => item.path === path), false, 'supplement scope changed: ' + path);
  const text = read(path); separateLocalEvidence.set(path, sha(text)); return text;
}
const correctionPath = 'runtime/evidence/C05-mcp-offline-fixture-correction-result.json';
const correction = JSON.parse(supplemental(correctionPath));
assert.equal(correction.status, 'three_fixture_stage_corrections_verified_locally');
assert.deepEqual(correction.sourceAndBuild, proof.sourceAndBuild);
const focusedRunnerPath = 'runtime/' + correction.focused.runner, focusedLogPath = 'runtime/' + correction.focused.log;
assert.equal(focusedRunnerPath, 'runtime/evidence/C05-mcp-offline-related-native-fixes1.json');
assert.equal(focusedLogPath, 'runtime/evidence/C05-mcp-offline-related-native-fixes1.log');
const focusedRunner = JSON.parse(supplemental(focusedRunnerPath)), focusedLog = supplemental(focusedLogPath);
const focusedCounts = logCounts(focusedLog);
assert.equal(correction.focused.stage, focusedRunner.stage); assert.equal(correction.focused.exitCode, 0);
assert.equal(correction.focused.sourceUnchanged, true); assert.deepEqual(correction.focused.counts, focusedCounts);
assert.equal(focusedCounts.pass, focusedCounts.tests); assert.ok(focusedCounts.tests > 0);
for (const field of ['fail', 'cancelled', 'skipped', 'todo']) assert.equal(focusedCounts[field], 0);
assert.match(focusedRunner.node, /^v24\./); assert.equal(focusedRunner.node, local.build.node);
assert.match(focusedRunner.command, /(?:^|\/)node-v24\.[^/]+\/bin\/node$/); assert.ok(focusedRunner.args.includes('--test'));
assert.equal(focusedRunner.status, 'passed'); assert.equal(focusedRunner.exitCode, 0); assert.equal(focusedRunner.signal, null);
assert.equal(focusedRunner.leaderExit.code, 0); assert.equal(focusedRunner.leaderExit.signal, null);
assert.equal(focusedRunner.stdioClose.code, 0); assert.equal(focusedRunner.stdioClose.signal, null);
assert.equal(focusedRunner.timedOut, false); assert.deepEqual(focusedRunner.errors, []);
assert.equal(focusedRunner.logCloseCompleted, true); assert.equal(focusedRunner.groupAbsentConfirmed, true);
assert.ok(Number.isFinite(Date.parse(focusedRunner.finishedAt)));
assert.equal(focusedRunner.sourceUnchanged, true);
for (const field of ['sourceBefore', 'sourceAfter']) assert.equal(focusedRunner[field], proof.sourceAndBuild.sourceDigest);
for (const field of ['buildBefore', 'buildAfter']) assert.deepEqual(focusedRunner[field], proof.sourceAndBuild);
assert.deepEqual(correction.files.map(item => item.path).sort(), ['src/tests/disclosure-boundaries.test.ts', 'src/tests/effect-proofs.test.ts', 'src/tests/model-boundaries.test.ts']);
for (const item of correction.files) {
  assert.equal(sha(bytes('runtime/' + item.path)), item.afterSha256);
  assert.ok(focusedRunner.args.includes(item.path.replace(/^src\//, 'dist/').replace(/\.ts$/, '.js')));
}
const localFixtureCorrection = { scope: 'separate_local_supplement_not_in_final_proof', result: correctionPath,
  runner: focusedRunnerPath, log: focusedLogPath, node: focusedRunner.node, exitCode: focusedRunner.exitCode,
  counts: focusedCounts, sourceAndBuild: correction.sourceAndBuild, finishedAt: focusedRunner.finishedAt,
...
const historyDetail = `첫 로컬 실행 이력: ${historyText}. new1은 당시 선택 파일의 통과 기록이며, related1의 실패는 context-dispatch의 종전 늦은 거절 기대를 새 조기 취소 경계에 맞춘 회귀와 구분한다. 원 입력·정책·dispatch 부재 검사는 유지했고 HTTP 입구 인수를 추가한 뒤 최종 소스로 다시 검증했다. 첫 실행의 지문·종료·로그를 바꾸거나 최종 통과 수에 더하지 않는다. 선행 [custody 증거](${root}/${predecessorPath})의 Linux ${pair(predecessorAll)}는 당시 결과로 보존한다.`;
const nativeFailureDetail = `첫 Linux 전체 회귀(attempt ${nativeAttemptHistory.nativeAttempt})는 ${number(firstNativeCounts.tests)}개 중 ${number(firstNativeCounts.pass)} 통과·${number(firstNativeCounts.fail)} 실패로 exit ${failedAll.exitCode} 종료했다(\`${prior.finishedAt}\`, ${firstNative.environment.node}). 당시 소스 \`${prior.sourceAndBuild.sourceDigest}\`, 빌드 파일 \`${prior.sourceAndBuild.filesDigest}\`, 파일 ${number(prior.sourceAndBuild.fileCount)}개다. 이 [실패 결과](${root}/${firstNativePath})와 [전체 원로그](${root}/${firstNativeLogPath})는 최종 proof의 priorAttempts·files에 보존되어 있다. 원로그의 실패 이름은 다음과 같다.\n\n${failureNames.map(name => '- ' + name).join('\n')}`;
const fixtureCorrectionDetail = `새 dispatch 전 입력 읽기에 시험의 첫 읽기 hook이 먼저 걸린 경계였다. 세 시험 파일의 기존 전송 전 제어 사례를 정확한 원 입력·저장된 running call·model-dispatch 영수증과 digest에 묶었고, reserved 읽기는 즉시 반환하도록 교정했다. 원래 호출 0·사용량 정산·사용자 상태·입력 보존 검사를 유지했고, effect proof의 dispatch 이전 소실은 별도 사례로 추가했다. 제품 가드는 변경하지 않았다. [교정 기록](${root}/${correctionPath}), [집중 시험 runner](${root}/${focusedRunnerPath}), [집중 원로그](${root}/${focusedLogPath})에서 ${focusedRunner.node}·exit ${focusedRunner.exitCode}·${pair(focusedCounts.tests)} 통과와 실행 전후 같은 최종 소스/빌드 지문을 대조했다. 이 세 파일은 updater가 별도로 확인한 로컬 보완 증거이며 최종 proof.files에 포함된 증거가 아니다. 교정 기록의 pending·nativeRecheck 문구는 그 기록 작성 당시 상태로 남긴다. 집중 수를 최종 신규·관련·Linux 전체 수에 더하지 않는다.`;
updates.set(resultPath, prepend(resultPath, `${intro}\n\n${entry}\n\n${evidenceDetail}\n\n${historyDetail}\n\n## 첫 Linux 실패와 시험 경계 교정\n\n${nativeFailureDetail}\n\n${fixtureCorrectionDetail}\n\n${cost}\n\n아래의 “진행 중”·Linux 미확정 표시는 작성 당시 초안이다. 최종 확인 범위는 이 상단의 실제 증거를 따르며 이전 진단·원로그는 보존한다.`, '## 구현 중 초안과 첫 검증 이력', '# C05 — MCP 서버 없는 일반 재개 결과'));
...
for (const [path, digest] of separateLocalEvidence) assert.equal(sha(bytes(path)), digest, 'local supplement changed before publication: ' + path);
...
console.log(JSON.stringify({ status: 'updated_from_final_proof', updated: targets, revision: 'v0.66', proof: proofPath, proofSha256: proofHash,
  ...
  nativeAttemptHistory, localFixtureCorrection,
  separateLocalEvidence: [...separateLocalEvidence].map(([path, sha256]) => ({ path, sha256 })),
