...
const base = realpathSync(mkdtempSync(join(tmpdir(), 'secumon-recovery-journal-cli-')));
...
try {
  ...
  console.log(JSON.stringify({ status: 'passed', backend: 'file-journal', personalMemory: 'documents',
    scope: 'actual local file effect, complete original journal/document-layout snapshot, public CLI prepare/status and preserved gate',
    changes: prepared.changes, counts: counts(), activation: prepared.activation, sourceArchiveUnchanged: true,
    restoredAgentUnchanged: true, preparedArchiveExact: true, ordinaryExecution: 'still_blocked', actualModelApi: 'not_run', installedBinary: 'not_run' }, null, 2));
} finally {
  const errors = [];
