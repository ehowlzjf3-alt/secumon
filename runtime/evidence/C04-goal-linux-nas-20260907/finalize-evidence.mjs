...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, requireCleanAudit } from './goal-c04-common.mjs';
...
assert.ok(/^v24\./.test(localBuildManifest.node), 'goal change local build requires Node 24');
...
function passedLocal(selected, pin) {
  ...
  assert.ok(observedExit.executions.some(item => item.sessionId === result.processEvidence.sessionId && item.exitCode === 0 &&
    `evidence/C04-goal-${item.stage}.log` === result.logPath));
...
const remoteNames = new Set(['goal-c04-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-goal-c04.mjs',
  'goal-c04-common.mjs', 'goal-c04-config.json', 'goal-c04-validation-inputs.json', 'goal-c04-targeted-files.json', 'goal-c04-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-goal-change-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
assert.equal(native.targeted.observedTestCount, nativeTests['new-goal-change-tests'].tests);
...
for (const name of ['configure.mjs', 'goal-c04-config.json', 'script-provenance.json', 'goal-c04-common.mjs', 'verify-linux-goal-c04.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C04', scope: 'explicit_general_goal_change_raw_receipts_control_and_compact',
  ...
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-goal-change-tests'],
