// C04 explicit-goal-change runner adapted from completed C04 registered-model; bounded process-group settlement and actual exits retained.
...
import { auditRoot, assertPin, assertLocalResult, configuration } from './goal-c04-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-goal-c04');
const expectedPin = JSON.parse(readFileSync(join(root, 'goal-c04-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_explicit_general_goal_change_raw_receipts_control_and_compact',
  runnerTemplate: 'evidence/C04-registered-linux-nas-20260907/verify-linux-registered-c04.mjs',
...
async function run(name, command, args, envOverrides = {}) {
  ...
  try {
    ...
    if (['new-goal-change-tests', 'related-existing-tests', 'all-tests'].includes(name)) {
      ...
      if (name === 'new-goal-change-tests') record.targeted.observedTestCount = step.tapSummary.tests;
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'goal-c04-validation-inputs.json'), 'utf8'));
  ...
  const lists = JSON.parse(readFileSync(join(root, 'goal-c04-targeted-files.json'), 'utf8'));
  ...
  await run('new-goal-change-tests', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
