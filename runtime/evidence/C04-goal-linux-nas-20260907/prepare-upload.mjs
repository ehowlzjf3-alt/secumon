...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, controlPath, remoteJson, requireCleanAudit } from './goal-c04-common.mjs';
...
const uploads = [[stamp + '.tar.gz', 'goal-c04-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-goal-c04.mjs', 'verify-linux-goal-c04.mjs'], [directory + '/goal-c04-common.mjs', 'goal-c04-common.mjs'],
  [stamp + '-validation-inputs.json', 'goal-c04-validation-inputs.json'],
  [directory + '/goal-c04-config.json', 'goal-c04-config.json'],
  [directory + '/targeted-files.json', 'goal-c04-targeted-files.json'], [directory + '/build-pin.json', 'goal-c04-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['goal-c04-source-attempt' + attempt + '.tar.gz', 'verify-linux-goal-c04.mjs', 'goal-c04-common.mjs', 'goal-c04-targeted-files.json', 'goal-c04-build-pin.json', 'goal-c04-validation-inputs.json', 'goal-c04-config.json']);
  ...
  const evidence = root + '/evidence-goal-c04';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-goal-c04-attempt' + attempt;
  const archive = root + '/goal-c04-source-attempt' + attempt + '.tar.gz';
