...
export class ExecutionRuntime {
  ...
  async settleStoredCollection(workId: string): Promise<Control | null> {
    ...
    if (expired) {
      ...
      if (!['cancelled', 'paused', 'failed', 'completed', 'blocked'].includes(state.status)) await this.readReconciliation.reconcile(workId, expired.id);
    ...
    for (const attempt of state.attempts) {
      if (!['failed', 'partial', 'cancelled', 'succeeded'].includes(attempt.status) || attempt.goalRevision !== state.goal.revision ||
        attempt.scope !== state.goal.scope || !attempt.readProgress || !attempt.readProgress.unknownCalls ||
