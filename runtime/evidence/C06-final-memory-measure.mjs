...
const metadataFiles = hostMetadataFiles(), initialMetrics = metadataFiles.diagnostics();
...
globalThis.fetch = async function (input, init) {
  ...
  const started = performance.now(), before = { ...counts }, timings = { ...inclusiveMs }, io = metadataFiles.diagnostics();
  ...
  const record = (status, failure = false) => {
    ...
    requests.push({ method: init?.method ?? 'GET', path, status, failure, elapsedMs: performance.now() - started,
      calls: delta(counts, before), inclusiveMs: delta(inclusiveMs, timings), metadataOperations: delta(metadataFiles.diagnostics(), io) });
  };
  try {
...
process.on('exit', () => {
  ...
  writeFileSync(process.env.SECUMON_MEASUREMENT_PATH, JSON.stringify({ sourceDigest: manifest.sourceDigest,
    ...
    inference: 'deterministic_fixture', actualModelCalls: 0, requests, counts, inclusiveMs, metadataOperations: delta(metadataFiles.diagnostics(), initialMetrics),
    caveats: ['single local run', 'method timings are inclusive and must not be summed', 'logical calls are not physical disk I/O', 'metadata counters are instrumented API operations; OS caches and physical disk I/O are not measured'] }, null, 2) + '\n');
});
