...
const recordPath = 'runtime/evidence/P3-continuation-boundary-local-verification.json';
const priorPath = 'runtime/evidence/P3-runtime-reconciliation-local-verification.json';
const priorSha256 = 'dfc9125bd06e4c1d58f91369f316e4bc3caa16700428ce0609eded09df4e6d4b';
const logPath = 'runtime/evidence/P3-continuation-boundary-verify.log';
const targetedPath = 'runtime/evidence/P3-continuation-boundary-targeted-final.log';
const planPath = 'design/chapters/P3-computer-continuation-plan.md';
const expectedPriorTests = 1771; const existingTargetedTests = 115; const expectedInnerFiles = 97;
...
const intermediate = walk('runtime/evidence').filter(p => /^runtime\/evidence\/P3-continuation-boundary-(?:typecheck|build|targeted|verify)-[^/]+\.log$/.test(p)).map(p => ({
...
(async () => {
  ...
  const workItemScopeValid = workItem?.status === 'in_progress' && workItem.verification?.local_contracts === 'partially_verified' &&
    ...
    workItem.plan === planPath && fs.existsSync(path.join(root, planPath)) && backlog.revision === 'v0.36' && verifiedWorkItems === 9;
  ...
  if (process.version !== 'v24.20.0' || metric(log, 'tests') !== expectedTestsPassed || testsPassed !== expectedTestsPassed || testsFailed !== 0 || !(durationMs > 0) ||
    prior.backlogRevision !== 'v0.35' || prior.testsPassed !== expectedPriorTests || additionalTests !== expectedAdditionalTests ||
  ...
  const record = { createdAt: new Date().toISOString(), workItems: ['P3-04'], status: 'verified_local_continuation_contracts_and_source_proof_boundaries',
    workItemStatus: workItem.status, localContracts: workItem.verification.local_contracts, backlogRevision: backlog.revision,
    plan: planPath,
    scope: 'Immutable continuation claims, single-successor CAS contracts, v1 source proof references protected through compaction and store reopen, retained-knowledge source proof closure, and outbox freshness gates. Production composition rejects continuation claims without an implemented proof checker. Actual continue/verify tools and a v2 continuation runner remain unsupported; this slice adds no cost experiment.',
    ...
    acceptanceEvidence: ['computer-continuation-contracts.test.ts', 'computer-continuation-unavailable.test.ts',
      'computer-continuation-context.test.ts', 'effect-proofs.test.ts', 'effect-proof-retention-outbox.test.ts',
      'effect-source-consumers.test.ts', 'knowledge-effect-custody.test.ts', 'computer-reconciliation-context.test.ts'].map(p => 'runtime/src/tests/' + p),
    artifacts: [targetedPath].map(evidence), intermediate, targeted: { ...evidence(targetedPath), existingTests: existingTargetedTests, newTests: additionalTests, testsPassed: targetedTests, testsFailed: 0 },
    actualDriver: 'not_run', actualModel: 'not_run', actualExternalModelCalls: 0, actualServiceCalls: 0, actualGuiInvoked: false,
    continuationExecution: { continueTool: 'unsupported', verifyTool: 'unsupported', runnerV2: 'unsupported', processKillContinuation: 'not_run',
      preservation: 'v1 parent metadata and original refs tested through five compactions and repository reopen with a contract-seeded claim and a test-only proof reader' },
    ...
    knownLimits: [
      'The current slice verifies persistent claim contracts and proof consumers only. No continue/verify tool, v2 checkpoint runner, remaining-step action or new continuation SIGKILL scenario was executed.',
      'The context fixture seeds a claim over real synthetic v1 parent artifacts with a test-only proof reader. Default production composition remains fail-closed for such claims.',
      'Repeated compact/reopen preserves metadata and original references; it does not establish current UI postconditions or successor completion.',
      'Continuation metadata and inherited historical observations do not copy raw actions into model input. Existing explicit current plan input remains a separate allowed input surface.',
      'Existing repository ports cannot freshly reauthenticate a source owner against the latest namespace or reviewer authority. Internal source custody/provenance checks and the current reader authority are distinct guarantees.',
      'Retained-knowledge proof closure is checked internally while current readers retain their own access boundary; internal validation does not grant the reader raw source access.',
      'Custody limits of 256 entries and 4 MiB apply per source-work closure, not as a shared budget across an entire multi-source query. Query-wide sharing and caching remain future optimization work.',
      'General knowledge-source blob integrity is not newly checked by the knowledge repository port. This closure checks canonical ledger metadata; computer effect artifacts and receipts have their own byte validation.',
      'A failed reconciliation record with a stored proof remains a proof dependency. Hiding an invalidated source for diagnostics does not authorize an executable resume packet.',
      'The single-owner synthetic driver and repository do not establish global cross-process leasing, real GUI exactly-once effects, power-loss safety or cryptographic UI attestation.',
      'No new cost comparison ran. Earlier synthetic cost artifacts are historical and are not measurements of the current driver or continuation design.',
      'Actual model quality, real UI latency, external services, OS/app/browser selection and production deployment remain unvalidated.',
      'Historical intermediate logs without an original hash are checked for existence only; their unchanged contents cannot be established from those records.'],
