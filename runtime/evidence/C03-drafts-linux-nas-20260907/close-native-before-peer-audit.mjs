...
async function audit() {
  ...
  const owned = execFileSync('/bin/ps', ['-eo', 'pid=,comm=,args='], { encoding: 'utf8' }).split('\n').flatMap(line => {
    const hit = line.trim().match(/^(\d+)\s+(\S+)\s+(.+)$/); if (!hit || Number(hit[1]) === process.pid) return [];
    return inside(link('/proc/' + hit[1] + '/exe')) || inside(link('/proc/' + hit[1] + '/cwd')) ? [Number(hit[1])] : [];
  });
  console.log(JSON.stringify({ verifiedAt: new Date().toISOString(), ownedProcesses: owned.length, ownedPids: owned,
    processAudit: 'all_comm_names_proc_executable_or_working_directory_within_dedicated_root_excluding_audit_self',
