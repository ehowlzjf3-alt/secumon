...
function inventory() {
  ...
  for (const name of readdirSync('/proc').filter(value => /^\d+$/.test(value))) {
    ...
    try {
      ...
      if (!Object.values(paths).some(inside)) continue;
      const stat = readFileSync(`/proc/${pid}/stat`, 'utf8'), fields = stat.slice(stat.lastIndexOf(')') + 2).split(' ');
      processes.push({ pid, ...paths, state: fields[0], parentPid: Number(fields[1]), processGroup: Number(fields[2]) });
    } catch (error) { if (!transient(error)) errors.push({ pid, code: error.code ?? error.name }); }
  }
  return { processes: processes.sort((a, b) => a.pid - b.pid), errors };
}
const beforeProcesses = inventory(); assert.deepEqual(beforeProcesses.errors, []);
assert.deepEqual(beforeProcesses.processes, [], 'Dedicated root must have no owned process except this runner');
...
try {
  ...
  process.exitCode = record.status === 'exited' && record.exit?.code === 0 && !record.terminationReason && record.errors.length === 0 &&
    record.afterProcesses?.processes.length === 0 && record.afterProcesses?.errors.length === 0 && record.pinUnchanged ? 0 : 1;
  console.log(JSON.stringify({ status: record.status, exit: record.exit ?? null, terminationReason: record.terminationReason,
    finishedAt: record.finishedAt, logBytes, pinUnchanged: record.pinUnchanged, ownedProcesses: record.afterProcesses?.processes.length ?? null, result: join(evidence, 'result.json') }));
}
