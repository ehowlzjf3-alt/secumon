...
export const directory = 'evidence/C05-mcp-recovery-linux-nas-20260907';
export const configuration = JSON.parse(readFileSync(new URL('./mcp-recovery-c05-config.json', import.meta.url), 'utf8'));
...
assert.equal(configuration.previousNativeResult, 'evidence-mcp-c05/result.json');
...
assert.ok(configuration.historyEvidence.includes('evidence/C05-mcp-linux-nas-20260907/verification.json'));
...
export function assertLocalResult(result, pin) {
  assert.equal(result?.sourceObservation?.kind, 'per_run_source_and_build_verification', 'MCP stored-result recovery run requires actual per-run source/build observations');
  assert.ok(/^v24\./.test(result.testNode), 'MCP stored-result recovery local tests require Node 24');
