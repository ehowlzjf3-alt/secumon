...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, requireCleanAudit } from './mcp-recovery-c05-common.mjs';
...
assert.ok(/^v24\./.test(localBuildManifest.node), 'MCP stored-result recovery local build requires Node 24');
...
function passedLocal(selected, pin) {
  ...
  assert.ok(observedExit.executions.some(item => item.sessionId === result.processEvidence.sessionId && item.exitCode === 0 &&
    `evidence/C05-mcp-recovery-${item.stage}.log` === result.logPath));
...
const remoteNames = new Set(['mcp-recovery-c05-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-mcp-recovery-c05.mjs',
  'mcp-recovery-c05-common.mjs', 'mcp-recovery-c05-config.json', 'mcp-recovery-c05-validation-inputs.json', 'mcp-recovery-c05-targeted-files.json', 'mcp-recovery-c05-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-mcp-stored-result-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
assert.deepEqual([...lists.newFiles].sort(), ['stored-tool-results', 'stored-result-runtime', 'mcp-stored-result', 'mcp-stored-result-recovery', 'stored-result-workflow'].map(name => `dist/tests/${name}.test.js`).sort());
assert.deepEqual(native.targeted.files, lists.newFiles); assert.deepEqual(native.related.files, lists.relatedFiles);
assert.equal(native.targeted.observedTestCount, nativeTests['new-mcp-stored-result-tests'].tests);
...
for (const name of ['configure.mjs', 'mcp-recovery-c05-config.json', 'script-provenance.json', 'mcp-recovery-c05-common.mjs', 'verify-linux-mcp-recovery-c05.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C05', scope: 'mcp_stored_result_recovery_general_entry',
  ...
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-mcp-stored-result-tests'],
  ...
  limitations: ['No real model/API, Knox/internal-service integration, or production deployment was tested.',
    ...
    'Only validated same-attempt plain MCP stored responses are restored; raw-only and intent-only gaps remain blocked. Post-send authority-change receipt/known-usage preservation and offline general-entry resume remain mandatory follow-up units.',
    'General-entry collection/page/wait recovery is not completed by this bounded plain-read stored-result recovery.',
