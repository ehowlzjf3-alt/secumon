// C05 MCP stored-result recovery runner adapted from completed C05 MCP general entry; bounded process-group settlement and actual exits retained.
...
import { auditRoot, assertPin, assertLocalResult, configuration } from './mcp-recovery-c05-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-mcp-recovery-c05');
const expectedPin = JSON.parse(readFileSync(join(root, 'mcp-recovery-c05-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_mcp_stored_result_recovery_general_entry',
  runnerTemplate: 'evidence/C05-mcp-linux-nas-20260907/verify-linux-mcp-c05.mjs',
...
async function run(name, command, args, envOverrides = {}) {
  ...
  const stageLimit = name === 'all-tests' ? record.executionLimits.fullStageDeadlineMs :
    ['new-mcp-stored-result-tests', 'related-existing-tests'].includes(name) ? record.executionLimits.testStageDeadlineMs : record.executionLimits.otherStageDeadlineMs;
  ...
  try {
    ...
    if (['new-mcp-stored-result-tests', 'related-existing-tests', 'all-tests'].includes(name)) {
      ...
      if (name === 'new-mcp-stored-result-tests') record.targeted.observedTestCount = step.tapSummary.tests;
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'mcp-recovery-c05-validation-inputs.json'), 'utf8'));
  ...
  const lists = JSON.parse(readFileSync(join(root, 'mcp-recovery-c05-targeted-files.json'), 'utf8'));
  const requiredNew = ['stored-tool-results', 'stored-result-runtime', 'mcp-stored-result', 'mcp-stored-result-recovery', 'stored-result-workflow'].map(name => `dist/tests/${name}.test.js`).sort();
  if (JSON.stringify([...lists.newFiles].sort()) !== JSON.stringify(requiredNew)) throw new Error('required_stored_result_tests_missing');
  ...
  await run('new-mcp-stored-result-tests', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
