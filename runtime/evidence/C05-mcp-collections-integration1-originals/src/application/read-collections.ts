...
import { assertReadWaitReady } from './read-waits.js';
...
export function createReadCollectionTool(binding: ReadCollectionBinding, runner: () => ReadCollections): Tool {
  ...
  return Object.freeze({ definition, execute: (task: TaskSpec, context: Context) => runner().execute(definition, source, task, context),
...
export class ReadCollections {
  ...
  private async publish(cp: ReadCheckpoint, expected: ArtifactRef | null, definition: ToolDefinition, task: TaskSpec, context: ExecutionContext, basis: WorkState,
    base: { head: ArtifactRef; checkpoint: ReadCheckpoint } | null, receivedResponse?: ReadResponse) {
    ...
    const current = async () => {
      ...
      if (!expected) await assertReadWaitReady(this.services, this.contracts, state, task);
  ...
  async execute(definition: ToolDefinition, source: ReadCollectionSource, task: TaskSpec, suppliedContext: Context): Promise<ToolResult> {
    ...
    if ((definition.collection?.pageValidation || definition.collection?.deferralValidation) && typeof context.authorize !== 'function') fail('read_authority_required');
    ...
    await assertReadWaitReady(this.services, this.contracts, state, task);
    if (state.attempts.find(a => a.id === context.attemptId)!.readProgress) fail('read_collection_already_started');
    const parentAttempt = this.parent(state, task, context.attemptId);
    const parent = parentAttempt ? await this.checkpoints.read(state, parentAttempt.id, parentAttempt.readProgress!.head) : null;
    ...
    if (now < (parent?.retryAt ?? 0)) fail('read_retry_not_due');
    ...
    let head = await this.publish(cp, null, definition, task, context, basis, parentAttempt ? { head: parentAttempt.readProgress!.head, checkpoint: parent! } : null);
