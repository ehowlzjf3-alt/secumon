...
export class ExecutionRuntime {
  ...
  #closing = false;
  #custodyOpen = true;
  constructor(readonly services: RuntimeServices, readonly tools: ToolContracts, readonly owner: string, readonly leaseMs = 30000, resultReuse?: ToolResultReuse) {
    ...
    this.readCheckpoints = new ReadCheckpoints(services, tools);
  ...
  control(state: WorkState): Control {
    return decideExecution(state, this.services.clock.now(), this.services.digester, task => this.tools.checkExecution(task, state.policy));
  }
  private requireCallable(state: WorkState, task: TaskSpec): void {
    const error = this.tools.checkExecution(task, state.policy); if (error) throw new Error(error);
  }
  ...
  async reserve(workId: string, taskId: string): Promise<Attempt> {
    ...
    if (selected) await assertReadWaitReady(this.services, this.tools, prepared, selected);
    ...
    const result = await this.change(workId, `reserve:${id}`, { taskId, id, owner: this.owner }, 'attempt_reserved', state => {
      ...
      if (selected) await assertReadWaitReady(this.services, this.tools, current, selected);
      if (!(await effectProofsCurrent(this.services, current))) throw new Error('effect_proof_unavailable');
      if (claim && selected) {
  ...
  async dispatch(workId: string, attemptId: string): Promise<boolean> {
    ...
    const result = await this.change(workId, `dispatch:${attemptId}`, { attemptId, owner: this.owner }, 'attempt_dispatched', state => {
      ...
      if (task) await assertReadWaitReady(this.services, this.tools, current, task);
  ...
  async execute(workId: string, attemptId: string): Promise<void> {
    ...
    try {
      ...
      const broker = new ToolBroker(this.services.state, this.tools, this.services.digester, this.services.clock, async state =>
        !this.#closing && await knowledgeInputsCurrent(this.services, state) && await sessionInputsCurrent(this.services, state), this.services.effects, this.services.continuations, this.services);
      const finished = (async () => {
        ...
        try {
          ...
          await this.receive(workId, attemptId, result, mode); return { error: null };
        }
  ...
  async recordStoredUsage(workId: string, attemptId: string, authorize?: (state: WorkState) => void): Promise<WorkState | null> {
    for (let retry = 0; retry < 8; retry++) {
      ...
      authorize?.(state);
      if (!this.storedUsages.candidate(state, attemptId)) return null;
      try {
        const ticket = await this.storedUsages.prepare(state, attemptId);
        authorize?.(state);
        ...
        const source = this.services.digester.digest(asJson({ receipt: ticket.receipt, usage: ticket.usage }));
        return (await this.change(workId, `tool-usage:${attemptId}:${source}`, { attemptId, source }, 'tool_execution_usage_recorded', next => {
          ...
          await this.storedUsages.assertCurrent(latest, ticket);
          authorize?.(await this.state(workId));
          if (!this.#custodyOpen) throw new Error('executor_closed');
        })).state;
      } catch (error) {
        if (!(error instanceof Error && ['stored_usage_changed', 'stored_usage_ticket_invalid'].includes(error.message))) throw error;
      }
    }
  ...
  async reconcileStoredUsages(workId: string, authorize: (state: WorkState) => void) {
    ...
    for (let retry = 0; retry < 8; retry++) {
      ...
      const candidates = entry.attempts.filter(attempt => this.storedUsages.candidate(entry, attempt.id)).map(attempt => attempt.id);
      try { selection = { entry, candidates, recorded: await recordedStoredUsageAttempts(this.services, entry, candidates) }; break; }
  ...
  async recover(workId: string, attemptId: string) {
    ...
    await this.readReconciliation.reconcile(workId, attemptId);
  ...
  async step(workId: string): Promise<Control> {
    ...
    const stored = await this.settleStoredResult(workId);
    if (stored) return stored;
    if (!['cancelled', 'paused', 'failed', 'blocked', 'completed'].includes(state.status)) {
      for (const attempt of state.attempts.filter(value => ['failed', 'partial', 'cancelled', 'succeeded'].includes(value.status) &&
        value.goalRevision === state.goal.revision && value.scope === state.goal.scope && value.readProgress &&
        value.readProgress.unknownCalls > 0 && !value.readProgress.successorAttemptId &&
        this.tools.get(value.toolId, value.toolVersion)?.tool.definition.collection?.responseRecovery))
        state = await this.readReconciliation.reconcile(workId, attempt.id);
    }
    ...
    if (control.kind === 'continue') {
      ...
      return control;
        ...
        if (['tool_connection_required', 'task_not_ready', 'attempt_not_dispatchable', 'attempt_expired', 'attempt_not_expired', 'result_not_pending', 'read_retry_not_due'].includes(code)) return this.control(await this.state(workId));
        throw error;
      }
    }
