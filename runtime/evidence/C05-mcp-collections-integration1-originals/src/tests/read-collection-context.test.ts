...
function assertMetadata(packet: ContextPacket, expected: ReturnType<typeof readCollectionContext>) {
  assert.deepEqual(packet.readCollections, expected); const json = JSON.stringify(packet);
...
for (const backend of adapters) for (const complete of [false, true]) test(`read collection context ${backend}: five compactions and reopen retain ${complete ? 'complete orphan' : 'partial'} resume metadata without page bodies`, async () => {
  await fixture(backend, async f => {
    ...
    for (let cycle = 1; cycle <= 5; cycle++) {
      ...
      assertMetadata(prepared.packet, expected); assert.equal(prepared.frame.memo.cycle, cycle); protectedDigests.add(prepared.frame.protectedDigest);
    ...
    try {
      ...
      const prepared = await compiler.prepare(state, { ...options, callId: 'after-restart' }); assertMetadata(prepared.packet, expected);
      assert.equal(prepared.frame.memo.cycle, 6); assert.equal(protectedDigests.has(prepared.frame.protectedDigest), true);
      const restored = await new ContextRecovery(services, contracts).restore('work-1', actor); assertMetadata(restored.packet.context, expected);
...
for (const backend of adapters) test(`read collection context ${backend}: lost derived frame regenerates while lost original raw response blocks context and recovery`, async () => {
  await fixture(backend, async f => {
    ...
    const regenerated = await f.compiler.prepare(current, { ...options, callId: 'regenerated' }); assertMetadata(regenerated.packet, readCollectionContext(current));
...
for (const backend of adapters) test(`read collection context ${backend}: resume metadata is mandatory input and cannot be evicted to satisfy a smaller byte budget`, async () => {
  await fixture(backend, async f => {
    ...
    assert.equal(estimates.length, 1); assertMetadata(estimates[0]!, readCollectionContext(state));
