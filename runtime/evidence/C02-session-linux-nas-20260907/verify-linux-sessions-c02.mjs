...
const evidence = join(root, 'evidence-sessions-c02');
const expectedPin = JSON.parse(readFileSync(join(root, 'sessions-c02-build-pin.json'), 'utf8')); 
...
try {
  ...
  await run('session-targeted', process.execPath, ['--test', '--test-concurrency=2', '--test-reporter=tap', 'dist/tests/session-flow.test.js', 'dist/tests/session-flow-kill.test.js', 'dist/tests/session-flow-boundaries.test.js', 'dist/tests/session-context-runtime.test.js', 'dist/tests/sqlite-sessions.test.js', 'dist/tests/persistent-session-presentation.test.js']);
