...
const directory = 'evidence/C02-session-linux-nas-20260907';
const root = '/home/shaneee/secumon-linux-test.pCJ0bd';
const control = '/tmp/secumon-session-nas.a3VADX/control';
const names = ['all-tests.log', 'architecture-cli-fixtures.log', 'architecture.log', 'build.log', 'fixtures.log', 'result.json', 'session-targeted.log', 'typecheck-core.log'];
mkdirSync(directory + '/final', { recursive: true });
execFileSync('scp', ['-o', 'ControlPath=' + control, ...names.map(name => 'nas:' + root + '/evidence-sessions-c02/' + name), directory + '/final/']);
...
rmdirSync('/tmp/secumon-session-nas.a3VADX');
const metadata = JSON.parse(readFileSync(directory + '/run-metadata.json', 'utf8'));
Object.assign(metadata, { status: native.status, finishedAt: native.finishedAt, sessionCompleted: true, logsCollected: names, sshClosed: true });
writeFileSync(directory + '/run-metadata.json', JSON.stringify(metadata, null, 2) + '\n');
console.log(JSON.stringify({ status: native.status, finishedAt: native.finishedAt, filesCollected: names.length, cleanup }));
