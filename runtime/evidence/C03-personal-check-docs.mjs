...
const root=resolve('..'),paths=['runtime/README.md','design/README.md','design/03-migration-plan.md','design/VERIFICATION.md','design/IMPLEMENTATION-RESUME.md','design/chapters/C03-personal-memory-plan.md','design/chapters/C03-personal-memory-progress.md','design/chapters/C03-personal-memory-result.md','design/chapters/C03-document-memory-plan.md','design/chapters/C03-postgres-adapter-notes.md','design/chapters/C03-personal-memory-cost-notes.md','design/secumon-review.html','design/implementation-backlog.json'];
...
for(const p of paths){const text=readFileSync(resolve(root,p),'utf8');documents.push({path:p,sha256:createHash('sha256').update(text).digest('hex')});
 ...
 if(p.endsWith('.html')){
  ...
  assert.match(text,/C03 개인 기억/);assert.match(text,/data-nas-run="29539"/);
 }
}
...
const report={checkedAt:new Date().toISOString(),status:issues.length?'failed':'passed',documents,localLinks:links.length,modulePaths,inlineScripts,issues};writeFileSync('evidence/C03-personal-doc-review.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({status:report.status,documents:documents.length,localLinks:links.length,modulePaths,inlineScripts,issues}));if(issues.length)process.exitCode=1;
