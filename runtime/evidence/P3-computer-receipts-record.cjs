...
const recordPath = 'runtime/evidence/P3-computer-receipts-local-verification.json';
const priorPath = 'runtime/evidence/P3-computer-use-local-verification.json';
const logPath = 'runtime/evidence/P3-computer-receipts-verify.log';
const targetedPath = 'runtime/evidence/P3-computer-receipts-targeted-4.log';
const groupCounts = { operationContracts: 5, driverReceipts: 16, storageFailure: 3, processRecovery: 2 };
...
const intermediate = walk('runtime/evidence').filter(p => /^runtime\/evidence\/P3-computer-receipts-(?:typecheck|build|targeted)-\d+\.log$/.test(p)).map(p => ({
...
(async () => {
  ...
  const workItemScopeValid = workItem?.status === 'in_progress' && workItem.verification?.local_contracts === 'partially_verified' &&
    ...
    backlog.revision === 'v0.34' && verifiedWorkItems === 9;
  if (process.version !== 'v24.20.0' || metric(log, 'tests') !== 1656 || testsPassed !== 1656 || testsFailed !== 0 || !(durationMs > 0) ||
    prior.testsPassed !== 1630 || prior.testsPassed + additionalTests !== testsPassed || metric(targeted, 'pass') !== 120 || metric(targeted, 'fail') !== 0 ||
    architecture.inspected !== 92 || !Array.isArray(architecture.failures) || architecture.failures.length || !fixture.passed || fixture.scenarios !== 4 ||
  ...
  const record = { createdAt: new Date().toISOString(), workItems: ['P3-04'], status: 'verified_local_computer_operation_receipts',
    workItemStatus: workItem.status, localContracts: workItem.verification.local_contracts, backlogRevision: backlog.revision,
    scope: 'Atomic bounded synthetic app/operation receipts, explicit read-only lookup, schema v1 migration, owner fencing and process/storage failure recovery. Runtime reconciliation and continuation are not yet integrated.',
    ...
    acceptanceEvidence: ['computer-operation-contracts.test.ts', 'synthetic-computer-receipts.test.ts', 'synthetic-computer-storage.test.ts',
      'computer-operation-recovery.test.ts'].map(p => 'runtime/src/tests/' + p),
    artifacts: [targetedPath].map(evidence), intermediate, actualExternalModelCalls: 0, actualServiceCalls: 0, actualGuiInvoked: false,
    performanceMeasuredThisSlice: false, historicalCost: { ...evidence(prior.cost.path), currentDriverComparison: false }, remaining: workItem.remaining,
    knownLimits: ['The synthetic app and receipts share a single atomic file; this does not establish exactly-once operation for real GUI drivers.',
      ...
      'Runtime reconciliation reservation/settlement and explicit remaining-step continuation remain required.',
      'The optional lookup port is not registered as an ordinary model tool or an unknown-effect bypass.',
