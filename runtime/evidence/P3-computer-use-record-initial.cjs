...
const costPath = 'runtime/evidence/P3-computer-use-cost.json';
const costScriptPath = 'runtime/evidence/P3-computer-use-cost.mjs';
const groupCounts = { contracts: 8, syntheticDriver: 14, runner: 24, context: 16, resultProof: 20, processRecovery: 4, validatorRegistration: 6 };
...
const intermediatePaths = [...walk('runtime/evidence').filter(p => /^runtime\/evidence\/P3-computer-(?:typecheck|build|targeted)-\d+\.log$/.test(p)),
  'runtime/evidence/P3-computer-use-cost.log'];
...
(async () => {
  ...
  const targeted = read('runtime/evidence/P3-computer-targeted-3.log').toString();
  if (process.version !== 'v24.20.0' || metric(log, 'tests') !== 1628 || testsPassed !== 1628 || testsFailed !== 0 || !(durationMs > 0) ||
    prior.testsPassed !== 1536 || prior.testsPassed + additionalTests !== testsPassed || metric(targeted, 'pass') !== 92 || metric(targeted, 'fail') !== 0 ||
  ...
  const record = { createdAt: new Date().toISOString(), workItems: ['P3-04'], status: 'verified_local_synthetic_computer_slice', workItemStatus: workItem.status,
    ...
    artifacts: [costPath, costScriptPath, 'runtime/evidence/P3-computer-use-cost.log', 'runtime/evidence/P3-computer-targeted-3.log'].map(evidence), intermediate,
