import type { ArtifactRef, Attempt, Evidence, TaskSpec, ToolResult, ToolUsage, WorkState } from '../domain/model.js';
...
export class ReadCheckpoints {
  ...
  private projection({ checkpoint, head, raw }: Verified): ToolResult {
    ...
    for (const key of Object.keys(usage) as (keyof ToolUsage)[]) {
      let sum: number | null = 0;
      for (const call of calls) {
        const value = (call.status === 'accepted' || call.status === 'deferred') && call.response ? raw.get(call.response.id)?.usage?.[key] : null;
        if (value === undefined || value === null) { sum = null; break; }
        sum += value; if (!Number.isSafeInteger(sum)) unavailable();
      }
      usage[key] = sum;
    }
