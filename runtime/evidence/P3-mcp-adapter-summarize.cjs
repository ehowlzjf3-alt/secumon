...
const directory = path.join(__dirname, 'mcp-adapter-final');
...
const logPath = path.join(__dirname, 'P3-mcp-adapter-targeted-final.log');
const log = fs.readFileSync(logPath, 'utf8');
assert.match(log, /^ℹ pass 57$/m); assert.match(log, /^ℹ fail 0$/m); assert.match(log, /^ℹ cancelled 0$/m);
...
write('P3-mcp-adapter-protocol-verification.json', { schemaVersion: 1, codeDigest, evidenceKind: 'local test assertions and instrumented SDK/fixture counters',
...
write('P3-mcp-adapter-recovery-verification.json', { schemaVersion: 1, codeDigest, targetedLog: pin(logPath),
  checks: { delayedReplyCancelledAfterHandler: true, hiddenCallRetriesInTestedScenarios: 0, explicitReconnectProcesses: 2,
  ...
  delayedReply: delayed, explicitReconnect: crashed, limits: ['Peer crash is fixture process.exit(23), not a worker SIGKILL or power-loss test.',
...
write('P3-mcp-adapter-shutdown-verification.json', { schemaVersion: 1, codeDigest,
