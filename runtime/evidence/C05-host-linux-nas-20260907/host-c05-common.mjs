...
export const directory = 'evidence/C05-host-linux-nas-20260907';
export const configuration = JSON.parse(readFileSync(new URL('./host-c05-config.json', import.meta.url), 'utf8'));
...
assert.equal(configuration.previousNativeResult, 'evidence-goal-c04/result.json');
...
assert.ok(configuration.historyEvidence.includes('evidence/C04-goal-linux-nas-20260907/verification.json'));
...
export function assertLocalResult(result, pin) {
  assert.equal(result?.sourceObservation?.kind, 'per_run_source_and_build_verification', 'host read-tool run requires actual per-run source/build observations');
  assert.ok(/^v24\./.test(result.testNode), 'host read-tool local tests require Node 24');
