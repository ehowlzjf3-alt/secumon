...
import { directory, root, configuration, assertPin, controlPath } from './host-c05-common.mjs';
...
const get = name => execFileSync('ssh', ['-S', control.socket, configuration.sshHost, '/bin/cat ' + root + '/evidence-host-c05/' + name], { timeout: 30000, maxBuffer: 32 * 1024 * 1024 });
...
const allowed = new Set(['build', 'new-host-tool-tests', 'related-existing-tests', 'typecheck-core', 'architecture', 'architecture-cli-fixtures', 'all-tests', 'fixtures']);
