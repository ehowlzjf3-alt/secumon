...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, controlPath, remoteJson, requireCleanAudit } from './host-c05-common.mjs';
...
const uploads = [[stamp + '.tar.gz', 'host-c05-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-host-c05.mjs', 'verify-linux-host-c05.mjs'], [directory + '/host-c05-common.mjs', 'host-c05-common.mjs'],
  [stamp + '-validation-inputs.json', 'host-c05-validation-inputs.json'],
  [directory + '/host-c05-config.json', 'host-c05-config.json'],
  [directory + '/targeted-files.json', 'host-c05-targeted-files.json'], [directory + '/build-pin.json', 'host-c05-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['host-c05-source-attempt' + attempt + '.tar.gz', 'verify-linux-host-c05.mjs', 'host-c05-common.mjs', 'host-c05-targeted-files.json', 'host-c05-build-pin.json', 'host-c05-validation-inputs.json', 'host-c05-config.json']);
  ...
  const evidence = root + '/evidence-host-c05';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-host-c05-attempt' + attempt;
  const archive = root + '/host-c05-source-attempt' + attempt + '.tar.gz';
