// C05 host-read-tool runner adapted from completed C04 explicit-goal-change; bounded process-group settlement and actual exits retained.
...
import { auditRoot, assertPin, assertLocalResult, configuration } from './host-c05-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-host-c05');
const expectedPin = JSON.parse(readFileSync(join(root, 'host-c05-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_host_read_tools_and_execution_authority',
  runnerTemplate: 'evidence/C04-goal-linux-nas-20260907/verify-linux-goal-c04.mjs',
...
async function run(name, command, args, envOverrides = {}) {
  ...
  try {
    ...
    if (['new-host-tool-tests', 'related-existing-tests', 'all-tests'].includes(name)) {
      ...
      if (name === 'new-host-tool-tests') record.targeted.observedTestCount = step.tapSummary.tests;
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'host-c05-validation-inputs.json'), 'utf8'));
  ...
  const lists = JSON.parse(readFileSync(join(root, 'host-c05-targeted-files.json'), 'utf8'));
  ...
  await run('new-host-tool-tests', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
