...
assert.equal(previousNativeResult, 'evidence-goal-c04/result.json', 'host read-tool validation reuses the completed goal-change dedicated root');
...
assert.ok(historyEvidence.includes('evidence/C04-goal-linux-nas-20260907/verification.json'), 'completed goal-change local proof must be retained as historical evidence');
const configPath = join(directory, 'host-c05-config.json'), controlPath = join(directory, 'control-directory.txt');
