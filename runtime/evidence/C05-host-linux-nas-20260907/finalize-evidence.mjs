...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, requireCleanAudit } from './host-c05-common.mjs';
...
assert.ok(/^v24\./.test(localBuildManifest.node), 'host read-tool local build requires Node 24');
...
function passedLocal(selected, pin) {
  ...
  assert.ok(observedExit.executions.some(item => item.sessionId === result.processEvidence.sessionId && item.exitCode === 0 &&
    `evidence/C05-host-${item.stage}.log` === result.logPath));
...
const remoteNames = new Set(['host-c05-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-host-c05.mjs',
  'host-c05-common.mjs', 'host-c05-config.json', 'host-c05-validation-inputs.json', 'host-c05-targeted-files.json', 'host-c05-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-host-tool-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
assert.equal(native.targeted.observedTestCount, nativeTests['new-host-tool-tests'].tests);
...
for (const name of ['configure.mjs', 'host-c05-config.json', 'script-provenance.json', 'host-c05-common.mjs', 'verify-linux-host-c05.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C05', scope: 'host_read_tools_and_execution_authority',
  ...
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-host-tool-tests'],
