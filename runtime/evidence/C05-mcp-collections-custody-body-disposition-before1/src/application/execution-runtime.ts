...
import { StoredReadUsages, type ReadUsageTicket } from './stored-read-usage.js';
...
export class ExecutionRuntime {
    ...
    for (let retry = 0; retry < 8; retry++) {
      ...
      try {
        ...
        return (await this.change(workId, `tool-usage:${attemptId}:${source}`, { attemptId, source }, 'tool_execution_usage_recorded', next => {
          ...
          else await this.storedReadUsages.assertCurrent(latest, ticket as ReadUsageTicket);
  ...
  async reconcileStoredUsages(workId: string, authorize: (state: WorkState) => void) {
    ...
    for (let retry = 0; retry < 8; retry++) {
      ...
      const candidates = entry.attempts.filter(attempt => plain.includes(attempt.id) || this.storedReadUsages.candidate(entry, attempt.id)).map(attempt => attempt.id);
