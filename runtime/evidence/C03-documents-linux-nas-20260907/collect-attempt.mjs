...
const get = name => execFileSync('ssh', ['-S', control, 'nas', '/bin/cat ' + root + '/evidence-documents-c03/' + name], { maxBuffer: 32 * 1024 * 1024 });
...
if (label === 'final' && result.status !== 'passed') throw new Error('final_collection_requires_pass');
if (!Array.isArray(result.steps) || result.steps.some(step => !/^[a-z][a-z0-9-]*$/.test(step.name))) throw new Error('invalid_step_name');
mkdirSync(directory + '/' + label, { mode: 0o700 }); writeFileSync(directory + '/' + label + '/result.json', bytes);
const names = ['result.json'];
for (const step of result.steps) {
  const name = step.name + '.log'; writeFileSync(directory + '/' + label + '/' + name, get(name)); names.push(name);
}
