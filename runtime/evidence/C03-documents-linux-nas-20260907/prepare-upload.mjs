...
const uploads = [['source.tar.gz', 'documents-c03-source.tar.gz'], ['verify-linux-documents-c03.mjs', 'verify-linux-documents-c03.mjs'],
  ['build-pin.json', 'documents-c03-build-pin.json']].map(([name, target]) => ({ local: directory + '/' + name, remote: root + '/' + target,
...
async function prepare(root, uploads, expected, attempt) {
  ...
  if (attempt > 1) {
    const previous = JSON.parse(fs.readFileSync(root + '/evidence-documents-c03/result.json'));
    if (previous.status !== 'failed' || !previous.finishedAt) throw new Error('previous_failure_not_finished');
    const archived = root + '/evidence-documents-c03-attempt' + (attempt - 1);
    if (fs.existsSync(archived)) throw new Error('previous_attempt_archive_exists');
    fs.renameSync(root + '/evidence-documents-c03', archived);
  }
  if (fs.existsSync(root + '/evidence-documents-c03')) throw new Error('evidence_already_exists');
  const cwd = root + '/runtime', backup = root + '/before-documents-c03' + (attempt === 1 ? '' : '-attempt' + attempt);
  ...
  execFileSync('/bin/tar', ['-xzf', root + '/documents-c03-source.tar.gz', '-C', cwd]);
  const apple = [];
