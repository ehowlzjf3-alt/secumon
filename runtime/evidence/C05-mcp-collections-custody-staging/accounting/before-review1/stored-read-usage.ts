...
export class StoredReadUsages {
  ...
  private scope(state: WorkState): Scope {
    const cache = new Map<string, { ref: ArtifactRef; bytes: Uint8Array; json?: unknown }>(); let bytesRead = 0;
    const read = async (value: ArtifactRef, policy: WorkState['policy'], maximum = maxRecordBytes) => {
      ...
      const bytes = await this.services.artifacts.get(structuredClone(ref), structuredClone(policy));
      ...
      if (bytes.byteLength !== ref.byteLength || Array.from(new Uint8Array(hash), value => value.toString(16).padStart(2, '0')).join('') !== ref.sha256)
        fail('stored_read_usage_original_unavailable');
      this.indexed(state, ref); bytesRead += bytes.byteLength;
      const result = { ref, bytes: new Uint8Array(bytes) }; cache.set(ref.id, result); return result;
    };
    return { store: {
