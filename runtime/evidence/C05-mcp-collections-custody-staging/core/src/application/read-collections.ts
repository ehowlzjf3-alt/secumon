...
function readAttemptIdentity(attempt: Attempt) {
  return { id: attempt.id, taskId: attempt.taskId, toolId: attempt.toolId, toolVersion: attempt.toolVersion,
    ...
    reuse: attempt.reuse ?? null, computerUse: attempt.computerUse ?? null };
}
...
export function snapshotReadCollectionBinding(binding: ReadCollectionBinding): ReadCollectionBinding {
  ...
  const supplied = binding.source;
  const fetch = supplied?.fetch, manifest = supplied?.manifest, validatePage = supplied?.validatePage,
    validateDeferral = supplied?.validateDeferral, restoreResponse = supplied?.restoreResponse, restoreUsage = supplied?.restoreUsage;
  if (!definition.collection || typeof fetch !== 'function' ||
    (definition.collection.kind === 'batch' && typeof manifest !== 'function')) fail('invalid_collection_source');
  if (validatePage !== undefined && typeof validatePage !== 'function' ||
    validateDeferral !== undefined && typeof validateDeferral !== 'function' ||
    restoreResponse !== undefined && typeof restoreResponse !== 'function' ||
    restoreUsage !== undefined && typeof restoreUsage !== 'function' ||
    manifest !== undefined && typeof manifest !== 'function') fail('invalid_collection_source');
  if (definition.collection.pageValidation && !validatePage) fail('tool_page_validator_required');
  if (definition.collection.deferralValidation && !validateDeferral) fail('tool_deferral_validator_required');
  if (definition.collection.responseRecovery && !restoreResponse) fail('tool_response_restorer_required');
  if (definition.collection.coverage && !manifest) fail('tool_read_manifest_required');
  const source: ReadCollectionSource = Object.freeze({ fetch: fetch.bind(supplied),
    ...(manifest ? { manifest: manifest.bind(supplied) } : {}),
    ...(validatePage ? { validatePage: validatePage.bind(supplied) } : {}),
    ...(validateDeferral ? { validateDeferral: validateDeferral.bind(supplied) } : {}),
    ...(restoreResponse ? { restoreResponse: restoreResponse.bind(supplied) } : {}),
    ...(restoreUsage ? { restoreUsage: restoreUsage.bind(supplied) } : {}) });
...
export class ReadCollections {
  ...
  async execute(definition: ToolDefinition, source: ReadCollectionSource, task: TaskSpec, suppliedContext: Context): Promise<ToolResult> {
    ...
    while (cp.phase === 'running') {
      ...
      if (brokerCustody) {
        const posted = intentReceipt?.state;
        if (!intentReceipt || !posted || intentReceipt.digest !== this.services.digester.digest(asJson({
          type: 'read_checkpoint_committed', data: { attemptId: context.attemptId, checkpointId: intentHead.id,
            phase: cp.phase, calls: cp.calls.length } })) || posted.id !== context.workId || posted.createdAt !== basis.createdAt ||
          !this.same(posted.policy, basis.policy) || !this.same(posted.goal, cp.goal) || dataGeneration(posted) !== cp.lifecycleGeneration ||
          posted.revision > state.revision || posted.updatedAt < cp.updatedAt ||
          posted.attempts.filter(value => value.id === context.attemptId).length !== 1) fail('read_intent_changed');
        const originalAttempt = this.guard(posted, definition, task, context, basis);
        const activeAttempt = state.attempts.find(value => value.id === context.attemptId)!;
        if (!this.same(readAttemptIdentity(originalAttempt), readAttemptIdentity(activeAttempt)) ||
          !this.same(originalAttempt.readProgress, this.progress(cp, intentHead)) ||
          posted.artifacts.filter(value => value.id === intentHead.id).length !== 1 ||
          !posted.artifacts.some(value => this.same(value, intentHead))) fail('read_intent_changed');
      }
