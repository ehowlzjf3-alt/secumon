...
writeFileSync('evidence/C02-compact-build-pin.json',JSON.stringify(pin,null,2)+'\n');
...
const uploads=[['source.tar.gz','compact-c02-source.tar.gz'],['verify-linux-compact-c02.mjs','verify-linux-compact-c02.mjs'],['../C02-compact-build-pin.json','compact-c02-build-pin.json']].map(([name,target])=>({local:directory+'/'+name,remote:root+'/'+target,sha256:createHash('sha256').update(readFileSync(directory+'/'+name)).digest('hex')}));
...
async function prepare(root,uploads,expected,attempt){
 ...
 if(attempt>1){const previous=JSON.parse(fs.readFileSync(root+'/evidence-compact-c02/result.json'));if(previous.status!=='failed'||!previous.finishedAt)throw new Error('previous_failure_not_finished');fs.renameSync(root+'/evidence-compact-c02',root+'/evidence-compact-c02-attempt'+(attempt-1));}
 if(fs.existsSync(root+'/evidence-compact-c02'))throw new Error('evidence_already_exists');
 ...
 const cwd=root+'/runtime',backup=root+'/before-compact-c02'+(attempt===1?'':'-attempt'+attempt);fs.mkdirSync(backup,{mode:448});
 ...
 execFileSync('/bin/tar',['-xzf',root+'/compact-c02-source.tar.gz','-C',cwd]);
