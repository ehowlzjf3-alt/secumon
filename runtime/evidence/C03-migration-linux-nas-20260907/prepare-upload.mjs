...
import { directory, root, assertPin, controlPath, remoteJson, requireCleanAudit } from './migration-c03-common.mjs';
...
const uploads = [[stamp + '.tar.gz', 'migration-c03-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-migration-c03.mjs', 'verify-linux-migration-c03.mjs'], [directory + '/migration-c03-common.mjs', 'migration-c03-common.mjs'],
  [stamp + '-validation-inputs.json', 'migration-c03-validation-inputs.json'],
  [directory + '/targeted-files.json', 'migration-c03-targeted-files.json'], [directory + '/build-pin.json', 'migration-c03-build-pin.json']]
...
for (const item of uploads) execFileSync('scp', ['-o', 'ControlPath=' + control.socket, item.local, 'nas:' + item.remote], { timeout: 120000 });
const result = remoteJson(async (root, uploads, expected, attempt) => {
  ...
  if (audit.auditErrors.length || audit.observedOwnedProcesses || audit.realRoot !== root || audit.rootMode !== '700' || audit.defaultNode !== 'v18.20.4') throw new Error('unsafe_prepare_environment');
  const allowed = new Set(['migration-c03-source-attempt' + attempt + '.tar.gz', 'verify-linux-migration-c03.mjs', 'migration-c03-common.mjs', 'migration-c03-targeted-files.json', 'migration-c03-build-pin.json', 'migration-c03-validation-inputs.json']);
  ...
  const evidence = root + '/evidence-migration-c03';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-migration-c03-attempt' + attempt;
  const archive = root + '/migration-c03-source-attempt' + attempt + '.tar.gz';
