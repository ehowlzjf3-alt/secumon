// D3 verification runner; inherited bounded process-group settlement from the frozen D2 runner.
...
import { auditRoot, assertPin } from './migration-c03-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-migration-c03');
const expectedPin = JSON.parse(readFileSync(join(root, 'migration-c03-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_personal_memory_sqlite_to_documents_backup_fence_seed_activation_recovery',
  runnerTemplate: 'evidence/C03-drafts-linux-nas-20260907/verify-linux-drafts-c03.mjs',
  executionLimits: { overallDeadlineMs: 1200000, nodeTestTimeoutMs: 60000, fullStageDeadlineMs: 900000, otherStageDeadlineMs: 180000,
...
async function run(name, command, args, envOverrides = {}) {
  ...
  const deadlineMs = Math.min(remaining, name === 'all-tests' ? record.executionLimits.fullStageDeadlineMs : record.executionLimits.otherStageDeadlineMs);
  ...
  try {
    ...
    if (['migration-new', 'migration-related', 'all-tests'].includes(name)) {
      ...
      if (name === 'migration-new') record.targeted.observedTestCount = step.tapSummary.tests;
      if (name === 'migration-related') record.related.observedTestCount = step.tapSummary.tests;
    }
  }
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'migration-c03-validation-inputs.json'), 'utf8'));
  ...
  for (const key of ['newTests', 'relatedTests']) {
    ...
    if (!result || result.code !== 0 || result.signal !== null || result.timedOut !== false || !result.finishedAt || JSON.stringify(result.sourceAndBuild) !== JSON.stringify(expectedPin)) throw new Error('local_validation_not_passed:' + key);
  }
  ...
  const lists = JSON.parse(readFileSync(join(root, 'migration-c03-targeted-files.json'), 'utf8'));
  const available = readdirSync(join(cwd, 'dist/tests')).filter(name => name.endsWith('.test.js')).sort();
  for (const [key, expectedCount] of [['newFiles', 5], ['relatedFiles', 35]]) {
    const files = lists[key];
    if (!Array.isArray(files) || files.length !== expectedCount || new Set(files).size !== files.length || files.some(file => !/^dist\/tests\/[a-z0-9-]+\.test\.js$/.test(file) || !available.includes(file.slice('dist/tests/'.length)))) throw new Error('invalid_targeted_files:' + key);
  }
  ...
  await run('migration-new', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
  await run('migration-related', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.relatedFiles]);
