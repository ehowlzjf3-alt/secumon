...
import type { ArtifactRef, Limits, Policy, TaskSpec, WorkState } from '../domain/model.js';
...
export function createCollectionEntryHost(options: CollectionEntryOptions, mode: 'online' | 'stored_only', barrier?: ResponseBarrier): AgentExecutionHost {
  ...
  const policy: Policy = { tenantId: 'mcp-collection-company', principalId: 'reader', allowWrites: false,
    allowedTools: ['fixture.collection'], allowedLabels: ['synthetic', 'public'], allowedDestinations: ['local'] };
  ...
  return { models: new Map([[MCP_AGENT_PROFILE, { execution: 'deterministic_fixture', async open(profile) {
    ...
    const turn: StructuredAgentTurnAdapter = new StructuredAgentTurnAdapter({ ...configuration, profile }, { async invoke(request) {
      ...
      if (evidence.every(value => value !== undefined)) {
        ...
        } else result = { kind: 'question', question: '현재 항목의 원 자료와 재개 가능한 계약을 확인해야 합니다.' };
      }
...
export async function readEntry(marker: CollectionEntryMarker) {
  ...
  try {
    ...
    return { state, receipts, input, latestInput, history: await stores.sessions.history(marker.scope, state.policy, { limit: 100 }),
