...
function completedLineage(f: Fixture, after: Awaited<ReturnType<typeof readEntry>>, transport: number) {
  ...
  const parent = state.attempts.find(value => value.id === f.marker.attemptId)!, child = state.attempts.find(value => value.id !== parent.id)!;
  ...
  const task = state.plan!.tasks.find(value => value.id === child.taskId)!;
  ...
  assert.equal(state.budget.used.toolCalls, f.before.state.budget.used.toolCalls + 1); assert.equal(state.budget.reservedToolCalls, 0);
  ...
  return { parent, child, checkpoint };
}
...
for (const backend of ['sqlite', 'file-journal'] as const) test(`${backend}: SIGKILL collection response resumes through actual CLI, compacts its real session and consumes an explicit complete successor offline`, { timeout: 60000 }, async t => {
  ...
  const done = await cli(f, 'resume'); assert.equal(done.run?.control.kind, 'complete', JSON.stringify(done));
  ...
  const after = await f.inspect(), { parent, checkpoint } = completedLineage(f, after, 0);
  ...
  const observed = entryObservations(f.options), compacts = observed.filter(value => value.kind === 'compact'), turns = observed.filter(value => value.kind === 'turn');
  ...
  assert.equal(turns.length, 2); assert.equal(turns[0]!.result.kind, 'plan'); assert.equal(turns[1]!.result.kind, 'answer');
  ...
  assert.equal(planning.session?.schemaVersion, 2); assert.equal(turns[1]!.input.packet.session?.schemaVersion, 2);
  let through = 0;
  ...
  assert.equal(after.state.budget.used.modelCalls, compacts.length + 2);
  assert.equal(after.state.budget.used.tokens, compacts.length * 150 + 2 * 250);
