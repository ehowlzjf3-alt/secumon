...
import { directory, root, configuration, assertPin, assertLocalResult, parseTestSummary, controlPath, remoteJson, requireCleanAudit } from './turn-c04-common.mjs';
...
const uploads = [[stamp + '.tar.gz', 'turn-c04-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-turn-c04.mjs', 'verify-linux-turn-c04.mjs'], [directory + '/turn-c04-common.mjs', 'turn-c04-common.mjs'],
  [stamp + '-validation-inputs.json', 'turn-c04-validation-inputs.json'],
  [directory + '/turn-c04-config.json', 'turn-c04-config.json'],
  [directory + '/targeted-files.json', 'turn-c04-targeted-files.json'], [directory + '/build-pin.json', 'turn-c04-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['turn-c04-source-attempt' + attempt + '.tar.gz', 'verify-linux-turn-c04.mjs', 'turn-c04-common.mjs', 'turn-c04-targeted-files.json', 'turn-c04-build-pin.json', 'turn-c04-validation-inputs.json', 'turn-c04-config.json']);
  ...
  const evidence = root + '/evidence-turn-c04';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-turn-c04-attempt' + attempt;
  const archive = root + '/turn-c04-source-attempt' + attempt + '.tar.gz';
