...
import { directory, root, configuration, assertPin, assertLocalResult, parseTestSummary, requireCleanAudit } from './turn-c04-common.mjs';
...
const localBuildManifest = JSON.parse(readFileSync(join(runtime, 'dist/build-manifest.json'), 'utf8'));
...
function passedLocal(selected, pin) {
  ...
  assert.ok(observedExit.observations.some(item => item.sessionId === result.processEvidence.sessionId && item.exitCode === 0 && item.logPath === result.logPath));
  if (result.processEvidence.path) {
  ...
  return { result: 'runtime/' + selected.path, log: 'runtime/' + result.logPath,
    ...
    exactFinishedAtCaptured: result.exactFinishedAtCaptured ?? !!result.finishedAt, processEvidence: result.processEvidence,
...
assert.deepEqual(json('evidence/C04-turn-local-build5-manifest.json'), localBuildManifest);
const finalizationDiagnosis = json('evidence/C04-turn-finalize-diagnosis.json');
...
const local = { platform: process.platform, arch: process.arch, node: process.version,
  nodeScope: 'finalization_process_only', testNodeVersion: 'not_captured_in_selected_local_results',
  build: { status: 'current_build_manifest_verified', node: localBuildManifest.node,
    manifest: 'runtime/evidence/C04-turn-local-build5-manifest.json', sourceAndBuild: pin },
...
const remoteNames = new Set(['turn-c04-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-turn-c04.mjs',
  'turn-c04-common.mjs', 'turn-c04-config.json', 'turn-c04-validation-inputs.json', 'turn-c04-targeted-files.json', 'turn-c04-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-agent-turn-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
assert.equal(native.targeted.observedTestCount, nativeTests['new-agent-turn-tests'].tests);
...
const historicalEvidence = configuration.historyEvidence.map(path => {
  ...
  return { path: 'runtime/' + path, sha256: files.get(path).sha256, status: record.status ?? null,
    interpretation: 'Operator-selected historical evidence; retained without promoting its pin, counts or diagnosis to this C04 run.' };
});
...
for (const name of ['configure.mjs', 'turn-c04-config.json', 'script-provenance.json', 'related-files-recommendation.json', 'turn-c04-common.mjs', 'verify-linux-turn-c04.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C04', scope: 'general_agent_turn_response_source_integrity_previous_draft_and_delivery',
  ...
  sourceAndBuild: pin, local, finalizationDiagnosis,
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-agent-turn-tests'],
  ...
  limitations: ['No real model/API, Knox/internal-service integration, or production deployment was tested.',
    ...
    'The historical D2 initialization failure and MCP stall remain bounded unresolved diagnoses; this C04 result does not establish their original causes.',
    'This script does not decide C04 chapter acceptance or complete the overall implementation goal.'],
