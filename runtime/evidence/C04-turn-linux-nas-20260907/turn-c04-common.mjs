...
export const directory = 'evidence/C04-turn-linux-nas-20260907';
export const configuration = JSON.parse(readFileSync(new URL('./turn-c04-config.json', import.meta.url), 'utf8'));
...
export function assertLocalResult(result, pin) {
  ...
  if (result.finishedAt !== undefined) {
    assert.ok(Number.isFinite(Date.parse(result.finishedAt))); assert.equal(result.timestampMeaning, 'actual_child_exit_observed_by_stage_runner');
  } else {
  ...
  for (const path of [result.logPath, result.sourceObservation.pinPath, result.sourceObservation.buildLogPath, result.processEvidence.path].filter(Boolean)) {
