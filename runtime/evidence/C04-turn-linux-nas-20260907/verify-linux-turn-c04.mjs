// C04 runner adapted from completed C03 D3; bounded process-group settlement and actual exits retained.
...
import { auditRoot, assertPin, assertLocalResult, configuration } from './turn-c04-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-turn-c04');
const expectedPin = JSON.parse(readFileSync(join(root, 'turn-c04-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_general_agent_turn_response_sources_previous_draft_session_compact_delivery',
  runnerTemplate: 'evidence/C03-migration-linux-nas-20260907/verify-linux-migration-c03.mjs',
...
async function run(name, command, args, envOverrides = {}) {
  ...
  try {
    ...
    if (['new-agent-turn-tests', 'related-existing-tests', 'all-tests'].includes(name)) {
      ...
      if (name === 'new-agent-turn-tests') record.targeted.observedTestCount = step.tapSummary.tests;
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'turn-c04-validation-inputs.json'), 'utf8'));
  ...
  const lists = JSON.parse(readFileSync(join(root, 'turn-c04-targeted-files.json'), 'utf8'));
  ...
  await run('new-agent-turn-tests', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
