...
files.push(...['memory-draft-web', 'personal-memory-drafts', 'personal-memory-draft-flow', 'personal-memory-draft-recovery',
  ...
  'session-flow-boundaries', 'session-context-runtime', 'persistent-session-presentation', 'sqlite-sessions', 'document-owner-registration-races'].map(name => `dist/tests/${name}.test.js`));
...
const child = spawn(process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...files], { stdio: 'inherit' });
