// C04 registered-model runner adapted from completed C04 window; bounded process-group settlement and actual exits retained.
...
import { auditRoot, assertPin, assertLocalResult, configuration } from './registered-c04-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-registered-c04');
const expectedPin = JSON.parse(readFileSync(join(root, 'registered-c04-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_registered_model_profile_general_entry_structured_compact',
  runnerTemplate: 'evidence/C04-window-linux-nas-20260907/verify-linux-window-c04.mjs',
...
async function run(name, command, args, envOverrides = {}) {
  ...
  try {
    ...
    if (['new-registered-model-tests', 'related-existing-tests', 'all-tests'].includes(name)) {
      ...
      if (name === 'new-registered-model-tests') record.targeted.observedTestCount = step.tapSummary.tests;
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'registered-c04-validation-inputs.json'), 'utf8'));
  ...
  const lists = JSON.parse(readFileSync(join(root, 'registered-c04-targeted-files.json'), 'utf8'));
  ...
  await run('new-registered-model-tests', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
