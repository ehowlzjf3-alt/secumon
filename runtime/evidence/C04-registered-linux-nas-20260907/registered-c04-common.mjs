...
export const directory = 'evidence/C04-registered-linux-nas-20260907';
export const configuration = JSON.parse(readFileSync(new URL('./registered-c04-config.json', import.meta.url), 'utf8'));
...
assert.equal(configuration.previousNativeResult, 'evidence-window-c04/result.json');
...
export function assertLocalResult(result, pin) {
  assert.equal(result?.sourceObservation?.kind, 'per_run_source_and_build_verification', 'registered run requires actual per-run source/build observations');
  assert.ok(/^v24\./.test(result.testNode), 'registered local tests require Node 24');
