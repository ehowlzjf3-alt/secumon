...
assert.equal(previousNativeResult, 'evidence-window-c04/result.json', 'registered model validation reuses the completed window dedicated root');
...
const configPath = join(directory, 'registered-c04-config.json'), controlPath = join(directory, 'control-directory.txt');
