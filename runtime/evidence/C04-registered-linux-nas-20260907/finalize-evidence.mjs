...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, requireCleanAudit } from './registered-c04-common.mjs';
...
assert.ok(/^v24\./.test(localBuildManifest.node), 'registered local build requires Node 24');
...
function passedLocal(selected, pin) {
  ...
  assert.ok(observedExit.executions.some(item => item.sessionId === result.processEvidence.sessionId && item.exitCode === 0 &&
    `evidence/C04-registered-${item.stage}.log` === result.logPath));
...
const remoteNames = new Set(['registered-c04-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-registered-c04.mjs',
  'registered-c04-common.mjs', 'registered-c04-config.json', 'registered-c04-validation-inputs.json', 'registered-c04-targeted-files.json', 'registered-c04-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-registered-model-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
assert.equal(native.targeted.observedTestCount, nativeTests['new-registered-model-tests'].tests);
...
for (const name of ['configure.mjs', 'registered-c04-config.json', 'script-provenance.json', 'registered-c04-common.mjs', 'verify-linux-registered-c04.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C04', scope: 'registered_model_profile_general_entry_structured_compact',
  ...
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-registered-model-tests'],
