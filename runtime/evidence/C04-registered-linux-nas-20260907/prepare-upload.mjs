...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, controlPath, remoteJson, requireCleanAudit } from './registered-c04-common.mjs';
...
const uploads = [[stamp + '.tar.gz', 'registered-c04-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-registered-c04.mjs', 'verify-linux-registered-c04.mjs'], [directory + '/registered-c04-common.mjs', 'registered-c04-common.mjs'],
  [stamp + '-validation-inputs.json', 'registered-c04-validation-inputs.json'],
  [directory + '/registered-c04-config.json', 'registered-c04-config.json'],
  [directory + '/targeted-files.json', 'registered-c04-targeted-files.json'], [directory + '/build-pin.json', 'registered-c04-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['registered-c04-source-attempt' + attempt + '.tar.gz', 'verify-linux-registered-c04.mjs', 'registered-c04-common.mjs', 'registered-c04-targeted-files.json', 'registered-c04-build-pin.json', 'registered-c04-validation-inputs.json', 'registered-c04-config.json']);
  ...
  const evidence = root + '/evidence-registered-c04';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-registered-c04-attempt' + attempt;
  const archive = root + '/registered-c04-source-attempt' + attempt + '.tar.gz';
