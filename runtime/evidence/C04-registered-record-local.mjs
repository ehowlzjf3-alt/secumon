...
import { parseTestSummary, assertLocalResult, assertLocalStageObservation } from './C04-registered-linux-nas-20260907/registered-c04-common.mjs';

const observations = JSON.parse(readFileSync('evidence/C04-registered-tool-exit-observations.json', 'utf8'));
const pin = await verifyEvaluationBuild(process.cwd());
const build = JSON.parse(readFileSync('evidence/C04-registered-build2.json', 'utf8'));
...
const pinPath = 'evidence/C04-registered-local-build2-pin.json';
writeFileSync(pinPath, JSON.stringify(pin, null, 2) + '\n', { flag: 'wx', mode: 0o600 });
for (const [name, stage] of [['new', 'new2'], ['related', 'related2']]) {
  const runnerPath = `evidence/C04-registered-${stage}.json`, logPath = `evidence/C04-registered-${stage}.log`;
  ...
  const result = { code: 0, signal: actual.signal, timedOut: actual.timedOut === true,
    ...
    processEvidence: { kind: 'exec_tool_and_stage_child_exit', sessionId: observed.sessionId,
      childPid: actual.pid, exitCode: observed.exitCode, path: 'evidence/C04-registered-tool-exit-observations.json' },
    sourceObservation: { kind: 'per_run_source_and_build_verification', perRunBeforeCaptured: true,
      pinPath, buildLogPath: 'evidence/C04-registered-build2.log', before: actual.buildBefore, after: actual.buildAfter, runnerPath },
    evidence: [[logPath, 'original_test_log'], [runnerPath, 'original_stage_runner_observation'],
      [pinPath, 'verified_final_build_pin'], ['evidence/C04-registered-build2.log', 'successful_build_log'],
      ['evidence/C04-registered-tool-exit-observations.json', 'recorded_exec_exit_observations']]
  ...
  writeFileSync(`evidence/C04-registered-${name}-result.json`, JSON.stringify(result, null, 2) + '\n', { flag: 'wx', mode: 0o600 });
