...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, selectedTestFiles, controlPath, remoteJson, requireCleanAudit } from './mcp-collections-c05-common.mjs';
...
const select = path => {
  const bytes = readFileSync(path), files = JSON.parse(bytes);
  assert.ok(Array.isArray(files) && files.length > 0 && new Set(files).size === files.length);
  for (const file of files) {
    assert.ok(typeof file === 'string' && /^dist\/tests\/[a-z0-9-]+\.test\.js$/.test(file));
    const stat = lstatSync(file); assert.ok(stat.isFile() && !stat.isSymbolicLink());
  }
  return { path, sha256: createHash('sha256').update(bytes).digest('hex'), files };
};
const selections = { newTests: select('evidence/C05-mcp-collections-new-files.json'), relatedTests: select('evidence/C05-mcp-collections-related-files.json') };
assert.equal(selections.newTests.files.some(file => selections.relatedTests.files.includes(file)), false);
const validationInputs = { pin, newTests: localProof(process.argv[3]), relatedTests: localProof(process.argv[4]), selections, selectedAt: new Date().toISOString() };
for (const key of ['newTests', 'relatedTests']) {
  const actual = JSON.parse(readFileSync(validationInputs[key].result.sourceObservation.runnerPath, 'utf8'));
  assert.deepEqual(selectedTestFiles(actual.args).sort(), [...selections[key].files].sort(), 'success observation did not execute the current selection');
}
const targeted = { newFiles: selections.newTests.files, relatedFiles: selections.relatedTests.files };
const preflight = JSON.parse(readFileSync(directory + '/preflight.json', 'utf8')); requireCleanAudit(preflight);
assert.equal(preflight.previousNative?.status, 'passed');
assert.equal(preflight.previousNative?.sha256, configuration.predecessor.nativeResultSha256);
assert.equal(createHash('sha256').update(readFileSync(configuration.predecessor.path)).digest('hex'), configuration.predecessor.sha256);
...
writeFileSync(stamp + '-targeted-files.json', JSON.stringify(targeted, null, 2) + '\n', { flag: 'wx', mode: 0o600 });
writeFileSync(directory + '/targeted-files.json', JSON.stringify(targeted, null, 2) + '\n', { mode: 0o600 });
...
const uploads = [[stamp + '.tar.gz', 'mcp-collections-c05-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-mcp-collections-c05.mjs', 'verify-linux-mcp-collections-c05.mjs'], [directory + '/mcp-collections-c05-common.mjs', 'mcp-collections-c05-common.mjs'],
  [stamp + '-validation-inputs.json', 'mcp-collections-c05-validation-inputs.json'],
  [directory + '/mcp-collections-c05-config.json', 'mcp-collections-c05-config.json'],
  [stamp + '-targeted-files.json', 'mcp-collections-c05-targeted-files.json'], [directory + '/build-pin.json', 'mcp-collections-c05-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['mcp-collections-c05-source-attempt' + attempt + '.tar.gz', 'verify-linux-mcp-collections-c05.mjs', 'mcp-collections-c05-common.mjs', 'mcp-collections-c05-targeted-files.json', 'mcp-collections-c05-build-pin.json', 'mcp-collections-c05-validation-inputs.json', 'mcp-collections-c05-config.json']);
  ...
  const evidence = root + '/evidence-mcp-collections-c05';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-mcp-collections-c05-attempt' + attempt;
  const archive = root + '/mcp-collections-c05-source-attempt' + attempt + '.tar.gz';
