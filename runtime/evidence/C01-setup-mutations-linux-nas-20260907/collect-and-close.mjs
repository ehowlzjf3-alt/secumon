...
const control = '/tmp/secumon-setup-mutations-nas.MPT0ot/control';
const names = ['all-tests.log', 'architecture-cli-fixtures.log', 'architecture.log', 'build.log', 'fixtures.log', 'result.json', 'setup-targeted.log', 'typecheck-core.log'];
mkdirSync(directory + '/final', { recursive: true });
execFileSync('scp', ['-o', 'ControlPath=' + control, ...names.map(name => 'nas:' + root + '/evidence-setup-mutations/' + name), directory + '/final/']);
...
rmdirSync('/tmp/secumon-setup-mutations-nas.MPT0ot');
