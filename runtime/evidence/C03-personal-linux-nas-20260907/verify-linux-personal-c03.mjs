...
const evidence = join(root, 'evidence-personal-c03');
const expectedPin = JSON.parse(readFileSync(join(root, 'personal-c03-build-pin.json'), 'utf8')); 
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  targetedExpectation: { files: 5, tests: 52 },
...
try {
  ...
  record.buildPin = await verifyEvaluationBuild(cwd); record.personalMemoryAsset = { path: 'dist/presentation/web/personal-memory.js', sha256: createHash('sha256').update(readFileSync(join(cwd, 'dist/presentation/web/personal-memory.js'))).digest('hex') }; persist();
  if (record.buildPin.sourceDigest !== expectedPin.sourceDigest) throw new Error('source_pin_mismatch');
  await run('personal-targeted', process.execPath, ['--test', '--test-concurrency=2', '--test-reporter=tap', 'dist/tests/personal-knowledge-service.test.js', 'dist/tests/personal-memory-context.test.js', 'dist/tests/personal-memory-presentation.test.js', 'dist/tests/sqlite-personal-knowledge.test.js', 'dist/tests/sqlite-knowledge-migration.test.js']);
