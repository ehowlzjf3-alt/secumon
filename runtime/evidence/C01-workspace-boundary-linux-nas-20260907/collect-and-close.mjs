...
const control = '/tmp/secumon-workspace-nas.EX2Gzb/control';
const names = ['all-tests.log', 'architecture-cli-fixtures.log', 'architecture.log', 'build.log', 'fixtures.log', 'result.json', 'workspace-targeted.log', 'typecheck-core.log'];
mkdirSync(directory + '/final', { recursive: true });
execFileSync('scp', ['-o', 'ControlPath=' + control, ...names.map(name => 'nas:' + root + '/evidence-workspace-boundary/' + name), directory + '/final/']);
...
rmdirSync('/tmp/secumon-workspace-nas.EX2Gzb');
