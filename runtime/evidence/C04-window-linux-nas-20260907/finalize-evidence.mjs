...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, requireCleanAudit } from './window-c04-common.mjs';
...
const remoteNames = new Set(['window-c04-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-window-c04.mjs',
  'window-c04-common.mjs', 'window-c04-config.json', 'window-c04-validation-inputs.json', 'window-c04-targeted-files.json', 'window-c04-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-context-window-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
assert.equal(native.targeted.observedTestCount, nativeTests['new-context-window-tests'].tests);
...
for (const name of ['configure.mjs', 'window-c04-config.json', 'script-provenance.json', 'window-c04-common.mjs', 'verify-linux-window-c04.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C04', scope: 'model_input_window_profile_inspection_bounded_compact',
  ...
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-context-window-tests'],
