...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, controlPath, remoteJson, requireCleanAudit } from './window-c04-common.mjs';
...
const uploads = [[stamp + '.tar.gz', 'window-c04-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-window-c04.mjs', 'verify-linux-window-c04.mjs'], [directory + '/window-c04-common.mjs', 'window-c04-common.mjs'],
  [stamp + '-validation-inputs.json', 'window-c04-validation-inputs.json'],
  [directory + '/window-c04-config.json', 'window-c04-config.json'],
  [directory + '/targeted-files.json', 'window-c04-targeted-files.json'], [directory + '/build-pin.json', 'window-c04-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['window-c04-source-attempt' + attempt + '.tar.gz', 'verify-linux-window-c04.mjs', 'window-c04-common.mjs', 'window-c04-targeted-files.json', 'window-c04-build-pin.json', 'window-c04-validation-inputs.json', 'window-c04-config.json']);
  ...
  const evidence = root + '/evidence-window-c04';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-window-c04-attempt' + attempt;
  const archive = root + '/window-c04-source-attempt' + attempt + '.tar.gz';
