// C04 window runner adapted from completed C04 turn; bounded process-group settlement and actual exits retained.
...
import { auditRoot, assertPin, assertLocalResult, configuration } from './window-c04-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-window-c04');
const expectedPin = JSON.parse(readFileSync(join(root, 'window-c04-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_model_input_window_profile_inspection_bounded_compact',
  runnerTemplate: 'evidence/C04-turn-linux-nas-20260907/verify-linux-turn-c04.mjs',
...
async function run(name, command, args, envOverrides = {}) {
  ...
  try {
    ...
    if (['new-context-window-tests', 'related-existing-tests', 'all-tests'].includes(name)) {
      ...
      if (name === 'new-context-window-tests') record.targeted.observedTestCount = step.tapSummary.tests;
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'window-c04-validation-inputs.json'), 'utf8'));
  ...
  const lists = JSON.parse(readFileSync(join(root, 'window-c04-targeted-files.json'), 'utf8'));
  ...
  await run('new-context-window-tests', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
