...
// node evidence/C05-mcp-select-local.mjs '<stages JSON>' '<observations JSON>'
...
function read(path, maximum = 2 * 1024 * 1024) {
  assert.ok(/^evidence\/C05-mcp-[a-zA-Z0-9-]+\.(json|log|mjs)$/.test(path), 'fixed local evidence path required');
...
const stagePath = stage => `evidence/C05-mcp-${stage}.json`;
const logPath = stage => `evidence/C05-mcp-${stage}.log`;
...
const pinPath = `evidence/C05-mcp-local-${selected.build}-pin.json`;
const processPath = 'evidence/C05-mcp-tool-exit-observations.json';
const encode = value => Buffer.from(JSON.stringify(value, null, 2) + '\n');
const prepared = new Map([[pinPath, encode(pin)]]);
read('evidence/C05-mcp-run.mjs'); read('evidence/C05-mcp-select-local.mjs');
prepared.set(processPath, encode({ recordedAt: new Date().toISOString(),
  ...
  helperSourceObservation: { meaning: 'Read at selection time; not claimed captured before each stage.',
    files: [inputs.get('evidence/C05-mcp-run.mjs'), inputs.get('evidence/C05-mcp-select-local.mjs')] } }));
for (const role of ['new', 'related']) {
  ...
  prepared.set(`evidence/C05-mcp-${role}-result.json`, encode({ code: actual.exitCode, signal: actual.signal,
