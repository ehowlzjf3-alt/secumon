...
assert.equal(previousNativeResult, 'evidence-host-c05/result.json', 'MCP general-entry validation reuses the completed host-tool dedicated root');
...
assert.ok(historyEvidence.includes('evidence/C05-host-linux-nas-20260907/verification.json'), 'completed host-tool local proof must be retained as historical evidence');
const configPath = join(directory, 'mcp-c05-config.json'), controlPath = join(directory, 'control-directory.txt');
