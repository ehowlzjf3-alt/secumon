...
import { directory, root, configuration, assertPin, controlPath } from './mcp-c05-common.mjs';
...
const get = name => execFileSync('ssh', ['-S', control.socket, configuration.sshHost, '/bin/cat ' + root + '/evidence-mcp-c05/' + name], { timeout: 30000, maxBuffer: 32 * 1024 * 1024 });
...
const allowed = new Set(['build', 'new-mcp-host-tests', 'related-existing-tests', 'typecheck-core', 'architecture', 'architecture-cli-fixtures', 'all-tests', 'fixtures']);
