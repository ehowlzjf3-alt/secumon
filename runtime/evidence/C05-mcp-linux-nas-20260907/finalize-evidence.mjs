...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, requireCleanAudit } from './mcp-c05-common.mjs';
...
assert.ok(/^v24\./.test(localBuildManifest.node), 'MCP general-entry local build requires Node 24');
...
function passedLocal(selected, pin) {
  ...
  assert.ok(observedExit.executions.some(item => item.sessionId === result.processEvidence.sessionId && item.exitCode === 0 &&
    `evidence/C05-mcp-${item.stage}.log` === result.logPath));
...
const remoteNames = new Set(['mcp-c05-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-mcp-c05.mjs',
  'mcp-c05-common.mjs', 'mcp-c05-config.json', 'mcp-c05-validation-inputs.json', 'mcp-c05-targeted-files.json', 'mcp-c05-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-mcp-host-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
assert.equal(native.targeted.observedTestCount, nativeTests['new-mcp-host-tests'].tests);
...
for (const name of ['configure.mjs', 'mcp-c05-config.json', 'script-provenance.json', 'mcp-c05-common.mjs', 'verify-linux-mcp-c05.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C05', scope: 'mcp_host_read_tools_general_entry',
  ...
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-mcp-host-tests'],
  ...
  limitations: ['No real model/API, Knox/internal-service integration, or production deployment was tested.',
    ...
    'Raw MCP response before result_received recovery, post-send authority-change receipt/known-usage preservation, and offline general-entry resume remain mandatory follow-up units.',
    'General-entry collection/page/wait recovery is not completed by this first plain-read connection.',
