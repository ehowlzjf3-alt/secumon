...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, controlPath, remoteJson, requireCleanAudit } from './mcp-c05-common.mjs';
...
const uploads = [[stamp + '.tar.gz', 'mcp-c05-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-mcp-c05.mjs', 'verify-linux-mcp-c05.mjs'], [directory + '/mcp-c05-common.mjs', 'mcp-c05-common.mjs'],
  [stamp + '-validation-inputs.json', 'mcp-c05-validation-inputs.json'],
  [directory + '/mcp-c05-config.json', 'mcp-c05-config.json'],
  [directory + '/targeted-files.json', 'mcp-c05-targeted-files.json'], [directory + '/build-pin.json', 'mcp-c05-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['mcp-c05-source-attempt' + attempt + '.tar.gz', 'verify-linux-mcp-c05.mjs', 'mcp-c05-common.mjs', 'mcp-c05-targeted-files.json', 'mcp-c05-build-pin.json', 'mcp-c05-validation-inputs.json', 'mcp-c05-config.json']);
  ...
  const evidence = root + '/evidence-mcp-c05';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-mcp-c05-attempt' + attempt;
  const archive = root + '/mcp-c05-source-attempt' + attempt + '.tar.gz';
