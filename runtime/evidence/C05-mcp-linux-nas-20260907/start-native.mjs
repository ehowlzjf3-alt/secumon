...
import { directory, root, configuration, assertPin, controlPath, remoteJson, requireCleanAudit } from './mcp-c05-common.mjs';
...
const command = '/usr/bin/env -i PATH=' + root + '/node-v24.20.0-linux-x64/bin:/usr/bin:/bin TMPDIR=' + root + '/tmp NODE_COMPILE_CACHE=' +
  root + '/tmp/node-compile-cache npm_config_cache=' + root + '/npm-cache /usr/bin/nice -n 10 ' + root + '/node-v24.20.0-linux-x64/bin/node ' + root + '/verify-linux-mcp-c05.mjs ' + root;
