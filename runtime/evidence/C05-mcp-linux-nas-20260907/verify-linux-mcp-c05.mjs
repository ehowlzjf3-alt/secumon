// C05 MCP general-entry runner adapted from completed C05 host-read-tool; bounded process-group settlement and actual exits retained.
...
import { auditRoot, assertPin, assertLocalResult, configuration } from './mcp-c05-common.mjs';
...
const cwd = join(root, 'runtime'), evidence = join(root, 'evidence-mcp-c05');
const expectedPin = JSON.parse(readFileSync(join(root, 'mcp-c05-build-pin.json'), 'utf8'));
...
const record = { schemaVersion: 1, status: 'running', startedAt: new Date().toISOString(),
  ...
  scope: 'native_linux_mcp_host_read_tools_general_entry',
  runnerTemplate: 'evidence/C05-host-linux-nas-20260907/verify-linux-host-c05.mjs',
...
async function run(name, command, args, envOverrides = {}) {
  ...
  const stageLimit = name === 'all-tests' ? record.executionLimits.fullStageDeadlineMs :
    ['new-mcp-host-tests', 'related-existing-tests'].includes(name) ? record.executionLimits.testStageDeadlineMs : record.executionLimits.otherStageDeadlineMs;
  ...
  try {
    ...
    if (['new-mcp-host-tests', 'related-existing-tests', 'all-tests'].includes(name)) {
      ...
      if (name === 'new-mcp-host-tests') record.targeted.observedTestCount = step.tapSummary.tests;
...
try {
  ...
  const validationInputs = JSON.parse(readFileSync(join(root, 'mcp-c05-validation-inputs.json'), 'utf8'));
  ...
  const lists = JSON.parse(readFileSync(join(root, 'mcp-c05-targeted-files.json'), 'utf8'));
  ...
  await run('new-mcp-host-tests', process.execPath, ['--test', '--test-concurrency=2', '--test-timeout=60000', '--test-reporter=tap', ...lists.newFiles]);
