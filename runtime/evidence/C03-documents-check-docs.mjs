...
for(const p of paths){const text=readFileSync(resolve(root,p),'utf8');documents.push({path:p,sha256:createHash('sha256').update(text).digest('hex')});
 ...
 if(p.endsWith('.html')){
  ...
  assert.match(text,/C03 문서 개인 기억/);assert.match(text,/data-nas-run="63469"/);
 }
}
...
const report={browserRender:'not_run_file_url_policy_blocked_not_bypassed',checkedAt:new Date().toISOString(),status:issues.length?'failed':'passed',documents,localLinks:links.length,modulePaths,inlineScripts,issues};writeFileSync('evidence/C03-documents-doc-review.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({status:report.status,documents:documents.length,localLinks:links.length,modulePaths,inlineScripts,issues}));if(issues.length)process.exitCode=1;
