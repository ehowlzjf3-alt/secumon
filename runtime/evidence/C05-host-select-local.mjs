...
// node evidence/C05-host-select-local.mjs '<stages JSON>' '<observations JSON>'
// stages: {build:"buildN",new:"newN",related:"relatedN",core:"coreN",architecture:"architectureN"}
// observations: {executions:[{stage,sessionId,exitCode}],synchronous?:[{stage,exitCode}]}
...
assert.equal(process.argv.length, 4, 'explicit stage selection and terminal observations JSON required');
for (const input of process.argv.slice(2)) assert.ok(Buffer.byteLength(input, 'utf8') <= 32 * 1024, 'bounded selection input');
const selected = JSON.parse(process.argv[2]), supplied = JSON.parse(process.argv[3]);
...
strictObject(supplied, Object.hasOwn(supplied ?? {}, 'synchronous') ? ['executions', 'synchronous'] : ['executions']);
...
function read(path, maximum = 2 * 1024 * 1024) {
  assert.ok(/^evidence\/C05-host-[a-zA-Z0-9-]+\.(json|log|mjs)$/.test(path), 'fixed local evidence path required');
...
const stagePath = stage => `evidence/C05-host-${stage}.json`;
const logPath = stage => `evidence/C05-host-${stage}.log`;
const stages = Object.fromEntries(roles.map(role => {
  ...
  assert.notEqual(actual.timedOut, true); assert.equal(actual.node, process.version);
  ...
  if (['build', 'core', 'architecture'].includes(role)) {
    ...
    assert.ok(actual.args.includes('--test') && actual.args.includes('--test-timeout=60000'));
...
// Same TAP/spec summary contract used by host-c05-common; no NAS configuration is opened.
...
const pinPath = `evidence/C05-host-local-${selected.build}-pin.json`;
const processPath = 'evidence/C05-host-tool-exit-observations.json';
const encode = value => Buffer.from(JSON.stringify(value, null, 2) + '\n');
const prepared = new Map([[pinPath, encode(pin)]]);
read('evidence/C05-host-run.mjs'); read('evidence/C05-host-select-local.mjs');
prepared.set(processPath, encode({ recordedAt: new Date().toISOString(),
  ...
  helperSourceObservation: { meaning: 'Read at selection time; not claimed captured before each stage.',
    files: [inputs.get('evidence/C05-host-run.mjs'), inputs.get('evidence/C05-host-select-local.mjs')] } }));
for (const role of ['new', 'related']) {
  ...
  prepared.set(`evidence/C05-host-${role}-result.json`, encode({ code: actual.exitCode, signal: actual.signal,
    // Derived from terminal success plus the runner's timeout flag; absent is not a captured false field.
    timedOut: actual.timedOut === true, testNode: actual.node, sourceAndBuild: pin, ...summaries[role], logPath: logPath(stage),
