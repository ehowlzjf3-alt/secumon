...
assert.equal(previousNativeResult, 'evidence-mcp-recovery-c05/result.json', 'custody validation requires the completed stored-result recovery predecessor');
...
assert.ok(historyEvidence.includes('evidence/C05-mcp-recovery-linux-nas-20260907/verification.json'), 'completed stored-result recovery proof must remain historical evidence');
const configPath = join(directory, 'mcp-custody-c05-config.json'), controlPath = join(directory, 'control-directory.txt');
