...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, selectedTestFiles, requireCleanAudit } from './mcp-custody-c05-common.mjs';
...
assert.ok(/^v24\./.test(localBuildManifest.node), 'MCP response custody local build requires Node 24');
...
function passedLocal(selected, pin) {
  ...
  assert.ok(observedExit.executions.some(item => item.sessionId === result.processEvidence.sessionId && item.exitCode === 0 &&
    `evidence/C05-mcp-custody-${item.stage}.log` === result.logPath));
...
const remoteNames = new Set(['mcp-custody-c05-source-attempt' + metadata.attempt + '.tar.gz', 'verify-linux-mcp-custody-c05.mjs',
  'mcp-custody-c05-common.mjs', 'mcp-custody-c05-config.json', 'mcp-custody-c05-validation-inputs.json', 'mcp-custody-c05-targeted-files.json', 'mcp-custody-c05-build-pin.json']);
...
const nativeTests = Object.fromEntries(['new-mcp-custody-tests', 'related-existing-tests', 'all-tests'].map(name => {
...
for (const [key, source, path] of [
  ['newFiles', 'newTests', 'evidence/C05-mcp-custody-new-files.json'],
  ['relatedFiles', 'relatedTests', 'evidence/C05-mcp-custody-related-files.json'],
]) {
...
assert.equal(native.targeted.observedTestCount, nativeTests['new-mcp-custody-tests'].tests);
...
for (const name of ['configure.mjs', 'mcp-custody-c05-config.json', 'script-provenance.json', 'mcp-custody-c05-common.mjs', 'verify-linux-mcp-custody-c05.mjs', 'finalize-evidence.mjs', 'collect-attempt.mjs', 'close-native.mjs', 'start-native.mjs', 'prepare-upload.mjs', 'preflight.mjs', 'select-local.mjs']) read(directory + '/' + name);
const result = { schemaVersion: 1, chapter: 'C05', scope: 'mcp_post_send_response_custody',
  ...
  nativeLinux: { status: 'passed', attempt: metadata.attempt, startedAt: native.startedAt, finishedAt: native.finishedAt,
    environment: native.environment, sourceAndBuild: native.buildPin, tests: nativeTests['all-tests'], newTests: nativeTests['new-mcp-custody-tests'],
  ...
  limitations: ['No real model/API, Knox/internal-service integration, or production deployment was tested.',
    ...
    'Profile startup still discovers registered providers; completely offline general-entry resume remains a required follow-up.',
    'General-entry collection/page/wait recovery is not completed by this bounded plain-read custody unit.',
