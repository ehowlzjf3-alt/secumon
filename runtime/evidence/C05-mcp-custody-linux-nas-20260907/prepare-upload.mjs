...
import { directory, root, configuration, assertPin, assertLocalResult, assertLocalStageObservation, parseTestSummary, selectedTestFiles, controlPath, remoteJson, requireCleanAudit } from './mcp-custody-c05-common.mjs';
...
const selections = { newTests: select('evidence/C05-mcp-custody-new-files.json'), relatedTests: select('evidence/C05-mcp-custody-related-files.json') };
...
const uploads = [[stamp + '.tar.gz', 'mcp-custody-c05-source-attempt' + attempt + '.tar.gz'],
  [directory + '/verify-linux-mcp-custody-c05.mjs', 'verify-linux-mcp-custody-c05.mjs'], [directory + '/mcp-custody-c05-common.mjs', 'mcp-custody-c05-common.mjs'],
  [stamp + '-validation-inputs.json', 'mcp-custody-c05-validation-inputs.json'],
  [directory + '/mcp-custody-c05-config.json', 'mcp-custody-c05-config.json'],
  [stamp + '-targeted-files.json', 'mcp-custody-c05-targeted-files.json'], [directory + '/build-pin.json', 'mcp-custody-c05-build-pin.json']]
...
const result = remoteJson(async (root, uploads, expected, attempt, expectedDefaultNode) => {
  ...
  const allowed = new Set(['mcp-custody-c05-source-attempt' + attempt + '.tar.gz', 'verify-linux-mcp-custody-c05.mjs', 'mcp-custody-c05-common.mjs', 'mcp-custody-c05-targeted-files.json', 'mcp-custody-c05-build-pin.json', 'mcp-custody-c05-validation-inputs.json', 'mcp-custody-c05-config.json']);
  ...
  const evidence = root + '/evidence-mcp-custody-c05';
  if (attempt > 1) {
  ...
  const cwd = root + '/runtime', backup = root + '/before-mcp-custody-c05-attempt' + attempt;
  const archive = root + '/mcp-custody-c05-source-attempt' + attempt + '.tar.gz';
