...
// node evidence/C05-mcp-custody-linux-nas-20260907/select-local.mjs evidence/C05-mcp-custody-stage-selection.json evidence/C05-mcp-custody-terminal-input.json
...
const suppliedInputs = process.argv.slice(2).map(path => {
  assert.ok(/^evidence\/C05-mcp-custody-[a-zA-Z0-9-]+\.json$/.test(path), 'bounded local recovery input path required');
...
function read(path, maximum = 2 * 1024 * 1024) {
  assert.ok(/^evidence\/C05-mcp-custody-[a-zA-Z0-9-]+\.(json|log|mjs)$/.test(path) || path === 'evidence/C05-mcp-custody-linux-nas-20260907/select-local.mjs', 'fixed local evidence path required');
  const absolute = resolve(runtime, path), stat = lstatSync(absolute);
...
const stagePath = stage => `evidence/C05-mcp-custody-${stage}.json`;
const logPath = stage => `evidence/C05-mcp-custody-${stage}.log`;
...
const listPaths = ['evidence/C05-mcp-custody-new-files.json', 'evidence/C05-mcp-custody-related-files.json'];
...
// Same TAP/spec summary contract used by mcp-custody-c05-common; no NAS configuration is opened.
...
const pinPath = `evidence/C05-mcp-custody-linux-nas-20260907/local-${selected.build}-pin.json`;
const processPath = 'evidence/C05-mcp-custody-linux-nas-20260907/tool-exit-observations.json';
const encode = value => Buffer.from(JSON.stringify(value, null, 2) + '\n');
const prepared = new Map([[pinPath, encode(pin)]]);
read('evidence/C05-mcp-custody-run.mjs'); read('evidence/C05-mcp-custody-linux-nas-20260907/select-local.mjs');
prepared.set(processPath, encode({ recordedAt: new Date().toISOString(),
  ...
  helperSourceObservation: { meaning: 'Read at selection time; not claimed captured before each stage.',
    files: [inputs.get('evidence/C05-mcp-custody-run.mjs'), inputs.get('evidence/C05-mcp-custody-linux-nas-20260907/select-local.mjs')] } }));
for (const role of ['new', 'related']) {
  ...
  prepared.set(`evidence/C05-mcp-custody-linux-nas-20260907/local-${role}-result.json`, encode({ code: actual.exitCode, signal: actual.signal,
