...
import { ContextCompiler } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/application/context-compiler.js';
import { ToolContracts } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/application/tool-contracts.js';
import { AjvSchemas } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/infrastructure/ajv-schemas.js';
import { RandomIds, Sha256Digester } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/infrastructure/digest.js';
import { FakeClock, FakeSink } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/infrastructure/fakes.js';
import { FileArtifactStore } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/infrastructure/file-artifacts.js';
import { StructuredPlannerAdapter } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/infrastructure/structured-planner.js';
import { initial, openRepository } from 'file:///Users/seunghanee/Documents/secumon/runtime/dist/tests/state-conformance-helpers.js';
...
const selectedIds = (options) => options.tools.map(tool => tool.id).sort();
async function fixture(t) {
    ...
    const estimates = [];
    const tools = [...mandatory, 'fixture.a', 'fixture.b'].map(id => ({
    ...
    const artifacts = { get: store.get.bind(store), exists: store.exists.bind(store),
        async put(value, attributes) { writes++; return store.put(value, attributes); } };
    const state = initial();
    state.policy.allowedTools = tools.map(tool => tool.definition.id);
    ...
    const planner = new StructuredPlannerAdapter({ identity: { provider: 'fixture', model: 'convergence', revision: '1' }, destination: 'local',
        ...
        inputEstimator: { profile: { id: 'optional-subset', revision: '1', templateRevision: '1', kind: 'conservative_estimate' },
            ...
            estimate(request, serialized) {
                ...
                estimates.push({ tools: selectedIds(options), tokens, bytes });
    ...
    const services = { state: repository, artifacts, planner, clock: new FakeClock(state.createdAt), ids: new RandomIds(),
...
async function proveIntermediateFit(f) {
    ...
    assert.equal(full.estimate.tokens, 180);
    assert.ok(full.estimate.bytes < limits.maxInputBytes);
    ...
    assert.equal(intermediate.tokens, 140);
    assert.ok(intermediate.tokens <= limits.maxInputTokens);
...
async function assertRetained(f, prepared, writesBefore) {
    ...
    assert.equal(prepared.estimate.tokens, 140);
    assert.ok(prepared.estimate.bytes <= limits.maxInputBytes);
    ...
    assert.equal(prepared.frame.memo.cycle, 1);
    assert.equal(f.writes(), writesBefore + 1);
...
test('prepare retains a measured fitting optional subset instead of exhausting unused-byte-budget retries', { timeout: 15000 }, async (t) => {
    ...
    let prepared;
    try {
        prepared = await f.compiler.prepare(f.state, limits);
    }
    catch (error) {
