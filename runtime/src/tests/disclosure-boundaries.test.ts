...
for (const adapter of adapters) {
  ...
  test(`${adapter}: disclosure revoked during stored model input read stops the final transport`, async t => {
    const store = new MemoryArtifactStore(); const entered = gate(); const release = gate(); let held = false;
    const f = await setup(t, adapter, 'local', store); const call = await f.planning.reserve(f.workId);
    const before = await f.execution.state(f.workId), originalInput = await store.get(call.inputArtifact, before.policy);
    f.services.artifacts = { put: store.put.bind(store), exists: store.exists.bind(store), async get(ref, policy) {
      const bytes = await store.get(ref, policy);
      if (!held && ref.id === call.inputArtifact.id) {
        assert.deepEqual(ref, call.inputArtifact);
        const current = await f.execution.state(f.workId), active = current.modelCalls.find(value => value.id === call.id);
    ...
    await Promise.race([entered.promise, running.then(() => assert.fail('post-dispatch input read was not reached'))]);
    ...
    await running; assert.equal(held, true); assert.equal(f.models.length, 0);
    const state = await f.execution.state(f.workId), received = state.modelCalls.find(value => value.id === call.id)!;
    assert.equal(received.status, 'received'); assert.equal(received.outcome, 'error'); assert.equal(received.usageStatus, 'reported');
    assert.equal(received.inputTokens, 0); assert.equal(received.outputTokens, 0);
    assert.equal(state.budget.reservedTokens, 0); assert.equal(state.budget.reservedModelCalls, 0);
    assert.equal(state.budget.used.modelCalls, 1); assert.equal(state.budget.used.tokens, 0); assert.equal(state.budget.used.unmeasuredModelCalls, 0);
    assert.deepEqual(received.inputArtifact, call.inputArtifact); assert.deepEqual(await store.get(call.inputArtifact, state.policy), originalInput);
    assert.deepEqual(state.goal, before.goal); assert.deepEqual(state.plan, before.plan); assert.deepEqual(state.contextHead, before.contextHead);
    assert.deepEqual(state.policy.disclosure!.destinations.find(d => d.destination === 'local')!.allowedLabels, ['synthetic']);
    assert.ok(await f.state.receipt(f.workId, `model-dispatch:${call.id}`));
