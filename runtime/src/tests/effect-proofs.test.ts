...
test('effect proofs: losing proof during pre-dispatch input read leaves the unsent reservation unchanged', async t => {
  const f = await fixture(t); const model = planning(f); const call = await model.reserve(f.workId); let fired = false;
  const before = (await f.state.get(f.workId))!, originalInput = await f.artifacts.get(call.inputArtifact, before.policy);
  f.services.artifacts = artifactsPort(f, { get: async (ref, policy) => {
    const bytes = await f.artifacts.get(ref, policy);
    if (!fired && ref.id === call.inputArtifact.id) {
      assert.deepEqual(ref, call.inputArtifact);
      const current = (await f.state.get(f.workId))!, active = current.modelCalls.find(value => value.id === call.id)!;
      assert.equal(active.status, 'reserved'); assert.equal(active.owner, call.owner);
      assert.equal(await f.state.receipt(f.workId, `model-dispatch:${call.id}`), null);
      fired = true; f.valid(false);
    }
    return bytes;
  } });
  await assert.rejects(model.execute(f.workId, call.id), /model_not_dispatchable/);
  assert.equal(fired, true); assert.equal(f.planner.inputs.length, 0);
  const next = (await f.state.get(f.workId))!, pending = next.modelCalls.find(value => value.id === call.id)!;
  assert.equal(pending.status, 'reserved'); assert.equal(pending.usageStatus, 'reserved'); assert.equal(pending.replyArtifact, null);
  assert.equal(next.budget.used.modelCalls, 0); assert.equal(next.budget.used.tokens, 0); assert.equal(next.budget.used.unmeasuredModelCalls, 0);
  assert.equal(next.budget.reservedModelCalls, before.budget.reservedModelCalls); assert.equal(next.budget.reservedTokens, before.budget.reservedTokens);
  assert.equal(next.budget.reservedModelCalls, 1); assert.equal(next.budget.reservedTokens, call.tokenReservation);
  assert.equal(await f.state.receipt(f.workId, `model-dispatch:${call.id}`), null);
  assert.equal(await f.state.receipt(f.workId, `model-receive:${call.id}`), null);
  assert.equal((await f.state.events(f.workId, 0)).some(event => event.type === 'model_call_dispatched'), false);
  assert.deepEqual(next, before); assert.deepEqual(await f.artifacts.get(call.inputArtifact, next.policy), originalInput);
});
...
test('effect proofs: losing proof during model input read prevents actual planner entry and still settles the dispatched call', async t => {
  ...
  f.services.artifacts = artifactsPort(f, { get: async (ref, policy) => {
    ...
    if (!fired && ref.id === call.inputArtifact.id) {
      assert.deepEqual(ref, call.inputArtifact);
      const current = (await f.state.get(f.workId))!, active = current.modelCalls.find(value => value.id === call.id);
      if (active?.status !== 'running') return bytes;
      const receipt = await f.state.receipt(f.workId, `model-dispatch:${call.id}`); assert.ok(receipt);
      assert.equal(receipt.digest, f.services.digester.digest({ type: 'model_call_dispatched', data: { callId: call.id, owner: call.owner } }));
      assert.equal(receipt.state.id, f.workId); assert.equal(receipt.state.revision, current.revision);
      assert.equal(active.owner, call.owner); assert.deepEqual(receipt.state.modelCalls.find(value => value.id === call.id), active);
      fired = true; f.valid(false);
    }
    return bytes;
  } });
  ...
  assert.equal(next.modelCalls[0]!.usageStatus, 'reported'); assert.equal(next.modelCalls[0]!.inputTokens, 0); assert.equal(next.modelCalls[0]!.outputTokens, 0);
  assert.equal(next.budget.reservedModelCalls, 0); assert.equal(next.budget.reservedTokens, 0); assert.equal(next.budget.used.unmeasuredModelCalls, 0);
  assert.deepEqual(next.modelCalls[0]!.inputArtifact, call.inputArtifact); assert.deepEqual(await f.artifacts.get(call.inputArtifact, next.policy), originalInput);
  assert.deepEqual(next.computerReconciliations, before.computerReconciliations); assert.deepEqual(next.goal, before.goal); assert.deepEqual(next.plan, before.plan);
  assert.ok(await f.state.receipt(f.workId, `model-dispatch:${call.id}`)); assert.equal(await model.adopt(f.workId, call.id), false);
});
