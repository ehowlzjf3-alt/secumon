...
for (const backend of ['sqlite', 'file-journal'] as const) {
  test(`${backend}: mixed host collection uses the profile's original stores and one compiled registry through answer and reopen`, { timeout: 60000 }, async t => {
    ...
    assert.deepEqual(Buffer.from(await profile.services.artifacts.get(h.fetches[0]!.raw, state.policy)), Buffer.from(h.fetches[0]!.bytes));
    ...
    assert.deepEqual(Buffer.from(await reopened.services.artifacts.get(h.fetches[0]!.raw, same.policy)), Buffer.from(h.fetches[0]!.bytes));
  });
}
