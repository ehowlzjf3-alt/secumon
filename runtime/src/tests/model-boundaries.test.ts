...
function heldInputRead() {
  ...
  let binding: { fixture: Awaited<ReturnType<typeof setup>>; call: ModelCall } | undefined;
  const artifacts: ArtifactStore = { put: store.put.bind(store), exists: store.exists.bind(store), async get(ref, policy) {
    ...
    if (held || !binding || !isDeepStrictEqual(ref, binding.call.inputArtifact)) return bytes;
    const { fixture: f, call } = binding;
    const current = await f.services.state.get(f.workId);
    const dispatched = current?.modelCalls.find(item => item.id === call.id);
    // A reserved input inspection must finish so dispatch can publish its receipt.
    if (dispatched?.status !== 'running') return bytes;
    assert.equal(current?.id, f.workId); assert.equal(dispatched.owner, call.owner);
    assert.deepEqual(dispatched.inputArtifact, call.inputArtifact);
    const receipt = await f.services.state.receipt(f.workId, `model-dispatch:${call.id}`);
    assert.ok(receipt);
    assert.equal(receipt.digest, f.services.digester.digest({ type: 'model_call_dispatched', data: { callId: call.id, owner: call.owner } }));
    assert.equal(receipt.state.id, f.workId);
    const originalDispatch = receipt.state.modelCalls.find(item => item.id === call.id);
    assert.equal(originalDispatch?.status, 'running'); assert.equal(originalDispatch?.owner, call.owner);
    assert.deepEqual(originalDispatch?.inputArtifact, call.inputArtifact);
    const value = JSON.parse(new TextDecoder().decode(bytes)) as Record<string, unknown>;
    assert.ok(value['packet'] && value['options']);
    held = true; entered.resolve(); await released.promise;
    return bytes;
  } };
  return { artifacts, entered, released, bind(fixture: Awaited<ReturnType<typeof setup>>, call: ModelCall) {
    assert.equal(binding, undefined); binding = { fixture, call: structuredClone(call) };
  } };
}
...
for (const change of ['cancel', 'goal', 'destination', 'labels'] as const) {
  test(`${change} during input artifact read prevents transport invocation and releases the known unused token reservation`, { timeout: 2000 }, async () => {
    ...
    const inputRef = structuredClone(call.inputArtifact), originalPolicy = (await f.execution.state(f.workId)).policy;
    const inputBytes = await f.services.artifacts.get(inputRef, originalPolicy);
    held.bind(f, call);
    const running = f.planning.execute(f.workId, call.id);
    try {
      await Promise.race([held.entered.promise, running.then(() => assert.fail('execution ended before the dispatched input read barrier'))]);
      const atBarrier = await f.services.state.get(f.workId);
      assert.equal(atBarrier?.modelCalls.find(item => item.id === call.id)?.status, 'running');
      const dispatchReceipt = await f.services.state.receipt(f.workId, `model-dispatch:${call.id}`);
      assert.ok(dispatchReceipt);
      assert.equal(dispatchReceipt.digest, f.services.digester.digest({ type: 'model_call_dispatched', data: { callId: call.id, owner: call.owner } }));
    ...
    const state = await f.execution.state(f.workId);
    assert.deepEqual(state.modelCalls.find(item => item.id === call.id)?.inputArtifact, inputRef);
    assert.deepEqual(await f.services.artifacts.get(inputRef, originalPolicy), inputBytes);
