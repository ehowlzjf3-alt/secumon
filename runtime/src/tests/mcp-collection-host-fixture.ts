...
export async function collectionHostFixture(t: TestContext, backend: Adapter = 'sqlite', deferral = false) {
  ...
  return { directory, auditPath, services, schemas, work, binding, config, context, options: options as Extract<McpHostToolsOptions, { config: McpStdioConfig }>, assembly, open, compose, accept, task, invoke, seed, audit,
