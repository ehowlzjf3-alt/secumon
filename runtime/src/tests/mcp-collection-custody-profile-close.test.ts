...
async function fixture(t: TestContext, backend: 'sqlite' | 'file-journal') {
  ...
  const host: AgentExecutionHost = { ...hostOptions, models: new Map([[MCP_AGENT_PROFILE, { execution: model.execution, async open(input) {
  ...
  const inspectStored = async () => {
    ...
    const stores = await openAgentStores(new FileAgentProfileStore(runtimeRoot), ready.root, undefined, hostOptions);
    try {
