...
export async function runAgentLifecycleCli(args: string[], profiles: AgentProfileStore, currentEngine: string,
  ...
  switch (positionals[0]) {
    ...
    case 'restore-status': result = inspectAgentRestoreReconciliation(profiles, values.directory, identityOptions); break;
    case 'restore-reconcile': {
      if (!values.offline) throw new AgentLifecycleError('lifecycle_offline_confirmation_required');
      const registration = hostOptions.restoreReconciliation;
      if (!registration || registration.sources.size === 0) throw new AgentLifecycleError('agent_restore_sources_required');
      result = await reconcileAgentRestore(profiles, { directory: values.directory, offline: true }, {
        ...identityOptions, sources: registration.sources,
