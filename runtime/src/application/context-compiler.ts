...
import type { ContextDecision, ContextGuidance, ContextHead, ContextItem, ContextObservation, ContextReadCollection, ContextRepresentation } from '../domain/context.js';
...
export class ContextCompiler {
  ...
  private canonicalCollectionsCurrent(packet: ContextPacket, collections: ContextReadCollection[]): boolean {
    const projected = packet.readCollections ?? [];
    if (projected.some(value => value.resumeMode !== undefined && value.resumeMode !== 'stored_complete')) return false;
    return this.digest(projected.map(({ resumeMode: _mode, ...original }) => original)) === this.digest(collections);
  }
  private resumeTask(state: WorkState, collection: ContextReadCollection, frontier: readonly TaskSpec[]): TaskSpec | null {
    const attempt = state.attempts.find(value => value.id === collection.attemptId);
    const task = frontier.find(value => value.id === collection.taskId), current = state.plan?.tasks.find(value => value.id === collection.taskId);
    if (!attempt || !task || !current || state.plan?.goalRevision !== state.goal.revision || this.digest(task) !== this.digest(current) ||
      attempt.taskId !== task.id || attempt.goalRevision !== state.goal.revision || attempt.scope !== state.goal.scope ||
      attempt.effect !== 'read' || attempt.effectState !== 'none' || attempt.adopted ||
      !['partial', 'failed', 'cancelled', 'succeeded'].includes(attempt.status) || !attempt.readProgress ||
      attempt.readProgress.phase !== 'complete' || attempt.readProgress.successorAttemptId !== null ||
      this.digest(attempt.readProgress.head) !== this.digest(collection.progress.head) ||
      taskDigest(task, this.services.digester) !== attempt.inputDigest || this.tools.check(task, state.policy)) return null;
    return task;
  }
  private completeResume(checkpoint: ReadCheckpoint, task: TaskSpec): boolean {
    return checkpoint.phase === 'complete' && checkpoint.collection.exhausted && !checkpoint.calls.some(call => call.status === 'intent') &&
      this.services.clock.now() >= (checkpoint.retryAt ?? 0) &&
      checkpoint.queryDigest === this.digest({ toolId: task.toolId, toolVersion: task.toolVersion, input: task.input });
  }
  private collectionContractsCurrent(packet: ContextPacket, state: WorkState): boolean {
    ...
    if (!this.canonicalCollectionsCurrent(packet, collections)) return false;
    const marked = (packet.readCollections ?? []).filter(value => value.resumeMode === 'stored_complete');
    if (marked.length) {
      const frontier = this.frontier(state);
      if (marked.some(value => !this.resumeTask(state, value, frontier) || !this.resumeTask(state, value, packet.plan?.tasks ?? []))) return false;
    }
  ...
  async sourcesCurrent(packet: ContextPacket, state: WorkState, signal?: AbortSignal, options: { ignoreDeliveryObligations?: boolean } = {}): Promise<boolean> {
    try {
      ...
      if (!this.canonicalCollectionsCurrent(packet, readCollections)) return false;
      const checkpoints = new ReadCheckpoints(this.services, this.tools);
      const frontier = (packet.readCollections ?? []).some(value => value.resumeMode !== undefined) ? this.frontier(state) : [];
      for (const [index, collection] of readCollections.entries()) {
        if (signal?.aborted) return false;
        const checkpoint = await checkpoints.read(state, collection.attemptId, collection.progress.head);
        if (packet.readCollections?.[index]?.resumeMode === 'stored_complete') {
          const task = this.resumeTask(state, collection, frontier);
          if (!task || !this.resumeTask(state, collection, packet.plan?.tasks ?? []) || !this.completeResume(checkpoint, task)) return false;
        }
      }
  ...
  private async compile(value: WorkState, limits: ContextLimits, inspecting: boolean): Promise<PreparedContext | ContextInspection> {
    ...
    const checkpoints = new ReadCheckpoints(this.services, this.tools);
    for (const collection of baseline.readCollections) {
      const task = this.resumeTask(state, collection, frontier);
      if (!task) { await read(collection.progress.head); continue; }
      // Reuse this authenticated checkpoint and the existing frontier, not another dispatch receipt or marker-only raw read.
      const checkpoint = await checkpoints.read(state, collection.attemptId, collection.progress.head); sourceReads++;
      if (this.completeResume(checkpoint, task)) collection.resumeMode = 'stored_complete';
    }
    ...
    if (inspecting) {
      ...
      for (let attempt = 0; ; attempt++) {
        ...
        budgetBytes = Math.max(0, Math.floor(Math.min(budgetBytes, chosen.bytes) * Math.min(limits.maxInputBytes / measured.bytes, limits.maxInputTokens / measured.tokens)) - 64);
      }
    }
    await this.fresh(state);
