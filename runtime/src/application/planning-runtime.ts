...
const PlanningCallInputSchema = AgentTurnCallInputSchema.omit({ turn: true }).extend({ packet: ContextPacketSchema });
export class PlanningRuntime {
  ...
  /** Only a marked planning reply needs collection proof; all marker decisions start with authenticated stored input bytes. */
  private async collectionReplyInput(state: WorkState, call: ModelCall): Promise<ContextPacket | null> {
    const ref = call.inputArtifact;
    if (ref.mediaType !== 'application/json' || ref.byteLength > 16 * 1024 * 1024) throw new Error('model_input_invalid');
    const bytes = await this.services.artifacts.get(ref, state.policy);
    if (bytes.byteLength !== ref.byteLength) throw new Error('model_input_invalid');
    const hash = await crypto.subtle.digest('SHA-256', new Uint8Array(bytes).buffer);
    if ([...new Uint8Array(hash)].map(byte => byte.toString(16).padStart(2, '0')).join('') !== ref.sha256) throw new Error('model_input_invalid');
    const input = PlanningCallInputSchema.parse(JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)));
    if (input.packet.workId !== state.id || input.packet.stateRevision !== call.baseStateRevision ||
      input.packet.goal.revision !== call.goalRevision || (input.packet.plan?.revision ?? 0) !== call.basePlanRevision ||
      input.options.callId !== call.id || input.options.maxOutputTokens !== call.maxOutputTokens) throw new Error('model_input_invalid');
    return input.packet.readCollections?.some(value => value.resumeMode === 'stored_complete') ? input.packet : null;
  }
  ...
  private async reserveModel(workId: string, compactRequest?: CompactRequestOptions): Promise<ModelCall | null> {
    ...
    const state = await this.execution.budgets.prepare(workId);
    const control = compact ? this.execution.control(state) : await this.execution.inspectedControl(state);
    assertExecutionAuthority(this.services, state);
  ...
  async adopt(workId: string, callId: string): Promise<boolean> {
    ...
    let collectionInput: ContextPacket | null = null;
    if (call.purpose !== 'agent_turn') {
      try { collectionInput = await this.collectionReplyInput(state, call); }
      catch { await this.reject(workId, callId, 'model_input_invalid'); return false; }
      if (collectionInput && !(await this.context.sourcesCurrent(collectionInput, state))) {
        await this.reject(workId, callId, 'model_collection_input_changed'); return false;
      }
    }
    ...
    try {
      ...
      await transact(this.services, workId, `model-adopt:${callId}`, 'model_plan_accepted', asJson(reply.proposal), next => {
        ...
        if (collectionInput && (this.services.digester.digest(asJson(this.call(current, callId).inputArtifact)) !== this.services.digester.digest(asJson(call.inputArtifact)) ||
          !(await this.context.sourcesCurrent(collectionInput, current)))) throw new Error('model_collection_input_changed');
  ...
  async compactStep(workId: string, options: { auto?: boolean; nextModelEnabled?: boolean } = {}): Promise<PlanningControl | null> {
    ...
    const control = await this.execution.inspectedControl(state);
  ...
  private async rejectReservedDisclosure(workId: string, callId: string): Promise<PlanningControl | null> {
    ...
    try {
      ...
      return await this.execution.inspectedControl(result.state);
    } catch (error) {
  ...
  async step(workId: string): Promise<PlanningControl> {
      ...
      if (pending.status === 'reserved' && pending.owner === this.owner) {
        try { await this.execute(workId, pending.id); }
          ...
          if (error instanceof Error && error.message === 'model_not_dispatchable') {
            ...
            const current = await this.execution.state(workId);
            if (current.policy.disclosure) return await this.execution.inspectedControl(current);
          }
    ...
    if (gate) {
      try {
        ...
        return await this.execution.inspectedControl(await this.execution.state(workId));
      }
    }
    try {
      ...
      if (['model_reservation_stale', 'model_not_needed', 'context_state_changed'].includes(code)) return await this.execution.inspectedControl(await this.execution.state(workId));
