...
function custodyAttempt(attempt: Attempt, collection = false) {
  return { id: attempt.id, taskId: attempt.taskId, toolId: attempt.toolId, toolVersion: attempt.toolVersion,
    ...
    ...(collection ? {} : { readProgress: attempt.readProgress ?? null }), reuse: attempt.reuse ?? null, computerUse: attempt.computerUse ?? null };
}
export class ToolBroker {
  constructor(readonly state: StateRepository, readonly tools: ToolContracts, readonly digester: Digester, readonly clock: Clock,
    ...
    readonly budgets?: Pick<RuntimeServices, 'budgetAuthority' | 'budgetChildren' | 'budgetLedgers' | 'executionAuthority'>,
    readonly readCollections?: ReadCollections) {
    if (readCollections && (readCollections.services.state !== state || readCollections.contracts !== tools ||
      readCollections.services.digester !== digester || readCollections.services.clock !== clock)) throw new BrokerError('read_collection_runtime_mismatch');
  }
  async invoke(workId: string, attemptId: string, owner: string, signal: AbortSignal, hooks: InvocationHooks = {}) {
    ...
    const local = !!this.readCollections?.isCompleteResumeCandidate(state, task, attemptId);
    if (local && this.readCollections!.owner !== owner) throw new BrokerError('read_collection_runtime_mismatch');
    const localEntry = local ? this.tools.get(task.toolId, task.toolVersion) : null;
    const executionError = () => local ? this.tools.check(task, state.policy) : this.tools.checkExecution(task, state.policy);
    const error = executionError(); if (error) throw new BrokerError(error);
    ...
    const contractCurrent = () => {
      ...
      if (local && entry !== localEntry) throw new BrokerError('tool_contract_changed');
      const error = executionError(); if (error) throw new BrokerError(error);
      return entry;
    };
    contractCurrent();
    if (this.beforeInvoke && !(await this.beforeInvoke(state))) throw new BrokerError('broker_knowledge_changed');
    const reused = local ? null : await hooks.reuse?.(state, task, attempt);
    if (!local && hooks.reuse && this.beforeInvoke && !(await this.beforeInvoke(state))) throw new BrokerError('broker_knowledge_changed');
    try { await assertBudgetAuthority({ state: this.state, digester: this.digester, clock: this.clock,
      budgetAuthority: this.budgets?.budgetAuthority, budgetChildren: this.budgets?.budgetChildren, budgetLedgers: this.budgets?.budgetLedgers }, state, {}, { kind: 'tool', task, attemptId }); }
    ...
    const authorize = async () => {
      ...
      try { await assertBudgetAuthority({ state: this.state, digester: this.digester, clock: this.clock,
        budgetAuthority: this.budgets?.budgetAuthority, budgetChildren: this.budgets?.budgetChildren, budgetLedgers: this.budgets?.budgetLedgers }, current, {}, { kind: 'tool', task: currentTask, attemptId }); }
      catch (error) { throw new BrokerError(error instanceof Error ? error.message : 'budget_grant_inactive'); }
    ...
    const collectionCustody = !local && definition.effect === 'read' && !definition.resultValidation &&
      definition.collection?.pageValidation === 'artifact-proof-v1' && definition.collection.responseRecovery === 'stored-response-v1' &&
      !!entry.tool.restoreReadUsage && !definition.reuse && !definition.computerContinuation && !definition.computerInputAssurance &&
      task.effect === 'read' && !task.computerResume && attempt.effect === 'read' && attempt.effectState === 'none' &&
      !attempt.reuse && !attempt.computerUse && !attempt.effectReceipt;
    let custodyActive = true;
    let authorizeResponseCustody: (() => Promise<void>) | undefined;
    if (plainCustody || collectionCustody) {
      const digest = (value: unknown) => custodyDigester.digest(asJson(value));
      const identity = (value: Attempt) => custodyAttempt(value, collectionCustody);
      ...
      const attemptPin = digest(identity(attempt));
      if (dispatched.state.id !== workId || dispatched.state.createdAt !== state.createdAt || dispatched.state.policy.tenantId !== tenantId ||
        ...
        !originalAttempt || originalAttempt.status !== 'running' || digest(identity(originalAttempt)) !== attemptPin ||
      ...
      authorizeResponseCustody = async () => {
        ...
        if (!current || current.id !== workId || current.createdAt !== original.state.createdAt || current.policy.tenantId !== tenantId || current.policy.principalId !== principalId ||
          ...
          !active || digest(identity(active)) !== attemptPin) throw new BrokerError('broker_response_custody_invalid');
    ...
    try {
      hooks.entered?.();
      const context = { workId, attemptId, policy: structuredClone(state.policy), signal, authorize,
        ...(authorizeResponseCustody ? { authorizeResponseCustody } : {}) };
      return local ? await this.readCollections!.consumeComplete(structuredClone(task), context) :
        await entry.tool.execute(structuredClone(task), context);
