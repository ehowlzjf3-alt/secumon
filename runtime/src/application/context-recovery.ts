...
export class ContextRecovery {
  ...
  private async refs(state: WorkState, deliveries: Delivery[]): Promise<ArtifactRef[]> {
    ...
    const required = [...state.attempts.flatMap(attempt => attempt.effectReceipt ? [attempt.effectReceipt.artifact] : []),
      ...
      ...state.attempts.flatMap(attempt => attempt.readProgress && (visibleReadProgress(state, attempt.readProgress) ||
        attempt.status === 'running' && attempt.goalRevision === state.goal.revision && attempt.scope === state.goal.scope) ? [attempt.readProgress.head] : [])]
    ...
    if (protectedOnly.size) {
      ...
      const reads = new StoredReadUsages(this.services, this.tools);
      for (const attempt of state.attempts) {
        if (excluded.size === protectedOnly.size) break;
        if (reads.candidate(state, attempt.id)) {
          const inspection = await reads.inspectCustody(state, attempt.id);
          if (inspection) for (const ref of inspection.custodyRefs) if (protectedOnly.has(ref.id)) excluded.add(ref.id);
          continue;
        }
    ...
    // Only authenticated, otherwise unused custody is projected out. Required evidence/result references still fail closed.
