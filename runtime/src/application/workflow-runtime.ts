...
export class WorkflowRuntime {
  ...
  async run(workId: string, actor: WorkActor, options: WorkflowRunOptions = {}): Promise<WorkflowRunResult> {
    ...
    while (compactSteps < maximum) {
      ...
      const stored = await this.execution.settleStoredResult(workId) ?? await this.execution.settleStoredCollection(workId);
    ...
    const finish = async (control: WorkflowControl, steps: number): Promise<WorkflowRunResult> => {
      for (let retry = 0; retry < 8; retry++) {
        ...
        const current = await this.execution.inspectedControl(state);
    ...
    for (let steps = compactSteps + 1; steps <= maximum; steps++) {
      ...
      const state = await this.authorize(workId, actor); const latest = await this.execution.inspectedControl(state);
