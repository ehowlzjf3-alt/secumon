...
실제 Windows 시험 호스트 또는 사내 MCP 연결 정보가 없어 goal은 입력 대기(`blocked`)다. 전체 목표와 완료 증거는 그대로 유지한다. 저장소의 안내·예시를 실제 접속 대상으로 사용하지 않으며, [외부 입력 대기 기록](external-acceptance-wait.json)을 기준으로 연결 정보가 제공된 범위부터 재개한다.
...
PG 정상 종료(status3), 원문·부분 백업·DB 보존을 확인했으며 전용 SSH master 종료와 로컬 control 디렉터리 제거도 확인했다. 실제 모델/API 중단을 유지하고 새 syscall·혼합 조합 검사를 추가하지 않는다.

[결과 설명](chapters/C10-postgres-capacity-result.md) · [결과 원본](../runtime/evidence/checkpoint407-capacity-result.json) · [실행 로그](../runtime/evidence/C10-postgres-capacity-launcher1.log) · [체크포인트](../runtime/evidence/checkpoint407.json) · [외부 인수 6분류](REMAINING-ACCEPTANCE.md)

## 이전 기록 — checkpoint406

아래의 남은 1묶음과 다음 작업 표현은 당시 기록이며 현재 상태는 위 checkpoint407을 따른다.

2026-09-09 · checkpoint406 · 기준선 `b9bb7b5`. **다음 필수 검사는 1묶음이다: 기존 V10-07의 PostgreSQL 전체 전달 64MiB 초과 거절·원자료/부분 백업 보존.** 유효한 pending 입력을 사용하며 적용하지 않은 입력을 적용 완료로 기록하지 않는다. 이 묶음 뒤 현재 환경에서 가능한 검증을 마감한다.

실제 PG 중단·재개는 SQLite 전체 실행 exit0, file-journal의 첫 이관 성공과 비교문 교정 뒤 엔진부터 재개 exit0으로 확인했다. 같은 fixture 판본의 전체 2회 실행이 아니다. 원자료·저장 이관 결과·실제 PG 원행·페이지 SHA·fence를 재확인해 완료한 이관을 재사용했다. 제품 소스·core 의존성 변경, 재빌드·전체 회귀 반복은 없다.

서버·DB pool·전용 SSH를 종료했고 원문·부분/완성 백업·DB·실패 기록을 보존했다. 마지막 과거 migration apply 거절 뒤 복원 DB에 남은 옛 operation fence는 임의 해제하지 않았다. 검증 성공을 그 DB의 일반 가동 가능 상태로 바꾸지 않는다.

그다음은 Windows, 운영 PG 역할·규모, 사내 MCP·Knox·A2A, 실제 앱, 운영 목표·파일럿의 외부 환경 인수다. 실제 모델/API 시험 중단을 유지한다. 모든 syscall·혼합 조합 같은 새 필수 검사를 계속 만들거나 완료한 전체 흐름을 반복하지 않는다. 전체 C10/goal은 진행 중이다.
...
시험 서버와 전용 SSH를 종료하고 원문·실패/성공 자료·백업·DB를 임시 경로에 보존했다. 실제 모델/API0, 운영 서비스 변경0이다. 임시 NAS 경로는 영구 보관을 보장하지 않는다.

[결과](chapters/C10-postgres-real-result.md) · [사용법](chapters/C10-postgres-real-usage.md) · [요구 대조](chapters/C10-postgres-real-audit.md) · [체크포인트](../runtime/evidence/checkpoint405.json) · [남은 확인](REMAINING-ACCEPTANCE.md)

**다음은 원래 남아 있던 실제 PG 중단·동시성·COMMIT 불명/호환 조건과 선택 운영 환경 인수다.** 이번 정상 흐름을 V10-06/07/08/18 전체 통과로 세지 않는다. 새 검증 항목을 만들거나 이미 통과한 정상 흐름·전체 로컬 시험을 반복하지 않는다. Windows, 사내 MCP/Knox/A2A·실제 앱과 운영 규모/복원 목표·시범 운영은 별도 환경이 필요하다. 전체 C10/goal은 진행 중이다.

## 이전 기록 — checkpoint404

아래는 당시 기록이며 현재 다음 작업은 checkpoint405를 따른다.

2026-09-09 · checkpoint404 · 기준선 `d4144a3`. **남았던 로컬 세 묶음을 완료했다:** 원자료→세션→개인기억→실행 기록, 기억 선택 만료/권한 오류·재선택·해제·재접속 화면, 문서형 기억의 반복 디렉터리 탐색 비용. macOS 관련61개+동일 HTTP 계측1개, NAS Linux 관련17개가 통과했다. 플랫폼 중복과 기준선 반복을 고유 시험으로 더하지 않는다.

문서 디렉터리 참조를 인스턴스당 최대64개 재사용하되 매번 현재 scope·권한·객체 동일성을 검사한다. 원문·snapshot 검사는 그대로다. 같은 합성 업무에서 문서 기억의 처리 시간23.132→13.317초, 디렉터리 검사6,601,410→3,568,230회였고 논리 조회·원문 읽기량은 같았다. 단회 관측이며 운영 응답 시간·용량 충족을 뜻하지 않는다.

두 저장소 구성에서 원문·출처·선택·모델/도구 원자료·답변·전달 영수증을 실제 바이트와 식별자로 연결했다. profile 재접속과 같은 실행 요청의 재전달은 기록을 보존하고 추가 모델·도구 호출0이었다. 실제 웹 화면은 기억 만료와 권한 오류를 구분했고 재선택·해제·권한 복원·HTTP 재접속 후 같은 업무를 복원했다. 화면 시험의 원문1개·상태revision6·모델/도구 사용0을 확인했다.

빌드·코어 타입 exit0, 구조204개·위반0. 최종 소스와 컴파일2,616개가 macOS/Linux에서 일치한다. 관리 시험·서버·임시 탭·전용 SSH 연결을 종료했다. 실제 모델/API 호출0이며 중단 결정을 유지한다. 전체 회귀는 반복하지 않았다.
...
아래는 당시 기록이며 현재 다음 작업은 checkpoint404를 따른다.
