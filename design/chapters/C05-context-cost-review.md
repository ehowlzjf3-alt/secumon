...
2026-09-08 · checkpoint375 현재 상태: 아래 후보 1의 한 원문 검사 안에서 이미 읽은 업무·영수증을 재사용하는 구현을 적용했고, 신규 7개와 기존 회귀 55개가 62/62 통과했다. 같은 계측 코드의 단회 SQLite `KnowledgeService.get`에서 `state.get`은 12회/28,404 JSON bytes→8회/18,936 bytes, `session.input`은 12회/16,824 bytes→8회/11,216 bytes였으며 `session.history` 4회·마지막 현재성 검사를 유지했고 기록한 결과 요약(본문 해시·revision·owner·size)이 같았다. 전체 반환 객체의 bytes 동일성을 주장하지 않는다. 이는 공개 포트 호출·반환 JSON 크기의 관측이며 시간·물리 I/O·모델 토큰 절감률이 아니다. 후보 2는 미채택이며 C06 진행을 막는 필수 잔여가 아니다. 아래 다른 비용 후보도 미구현·미계측 상태로 구분한다. collection 권한 차단 입구 4개·코어 6개와 CLI 현재 안내 회귀를 포함해 C05 고유 선택 682개가 통과했고 C03 고유 243개는 별도 집계다. [측정 비교](../../runtime/evidence/C05-source-read-comparison2.json) · [62개 원로그](../../runtime/evidence/C05-ordered-target11.log) · [checkpoint375 결과와 소스별 범위](C05-ordered-verification-result.md) · [collection 입구 결과·재허용 뒤 명시 재개 미검증 범위](C05-collection-permission-resume-result.md). 실제 모델/API 시험 중단을 유지하며, 아래 날짜별 검토·단회 기준선과 당시의 미구현 표현은 과거 기록으로 보존한다.
